...
/**
 * OpenAI-compatible ARD index enhancement provider.
 *
 * @author nacos
 */
@Service
public class OpenAiCompatibleArdIndexEnhancementService implements ArdIndexEnhancementService {
    ...
    static final String KEY_ENDPOINT = "nacos.ai.ard.index.enhancement.endpoint";
    
    static final String KEY_API_KEY = "nacos.ai.ard.index.enhancement.api-key";
    
    static final String KEY_MODEL = "nacos.ai.ard.index.enhancement.model";
    
    static final String KEY_TIMEOUT_MS = "nacos.ai.ard.index.enhancement.timeout-ms";
    
    static final String KEY_MAX_ITEMS = "nacos.ai.ard.index.enhancement.max-items-per-type";
    ...
    public OpenAiCompatibleArdIndexEnhancementService() {
        this(HttpClient.newBuilder().build());
    }
    
    OpenAiCompatibleArdIndexEnhancementService(HttpClient httpClient) {
    ...
    public List<ArdIndexEnhancementChunk> enhance(ArdEntry entry,
        List<ArdChunk> existingChunks) throws Exception {
    ...
    private String requestEnhancement(ArdEntry entry, List<ArdChunk> existingChunks,
        List<ArdIndexEnhancementContent> contents) throws Exception {
    ...
    Map<String, Object> resourcePayload(ArdEntry entry, List<ArdChunk> existingChunks,
        List<ArdIndexEnhancementContent> contents) {
    ...
    private List<Map<String, String>> chunkPayload(List<ArdChunk> existingChunks) {
        if (existingChunks == null || existingChunks.isEmpty()) {
            return Collections.emptyList();
        }
        List<Map<String, String>> result = new ArrayList<>();
        for (ArdChunk chunk : existingChunks) {
    ...
    List<ArdIndexEnhancementChunk> parseEnhancementContent(String content, String model) {
        ...
        addValue(chunks, ArdIndexConstants.CHUNK_TYPE_AI_SUMMARY, parsed.get("summary"),
            metadata);
        addValue(chunks, ArdIndexConstants.CHUNK_TYPE_BILINGUAL_ALIAS,
            parsed.get("bilingualAliases"), metadata);
        addValue(chunks, ArdIndexConstants.CHUNK_TYPE_CAPABILITY_SYNONYM,
            parsed.get("capabilitySynonyms"), metadata);
        addValue(chunks, ArdIndexConstants.CHUNK_TYPE_EXAMPLE_QUERY,
            parsed.get("exampleQueries"), metadata);
        return chunks;
    }
    
    private void addValue(List<ArdIndexEnhancementChunk> chunks, String chunkType, Object value,
        String metadata) {
        for (String text : toStringList(value)) {
            if (StringUtils.isNotBlank(text)) {
                chunks.add(new ArdIndexEnhancementChunk(chunkType, text.trim(), metadata));
            }
        }
    }
    ...
    String errorMessage(int statusCode, String body) {
        ...
        String message = "ARD index enhancement LLM request failed, status=" + statusCode;
