...
public class ServiceInfo {
    ...
    public static String getKey(String name, String clusters, String unit, boolean isAllIPs) {
        ...
        if (!isEmpty(clusters) && !isEmpty(unit)) {
            return isAllIPs ? name + SPLITTER + clusters + SPLITTER + unit + SPLITTER + ALL_IPS
                : name + SPLITTER + clusters + SPLITTER + unit;
        }

        if (!isEmpty(clusters)) {
            return isAllIPs ? name + SPLITTER + clusters + SPLITTER + ALL_IPS : name + SPLITTER + clusters;
        }

        if (!isEmpty(unit)) {
            return isAllIPs ? name + SPLITTER + EMPTY + SPLITTER + unit + SPLITTER + ALL_IPS :
                name + SPLITTER + EMPTY + SPLITTER + unit;
        }

        return isAllIPs ? name + SPLITTER + ALL_IPS : name;
    }
