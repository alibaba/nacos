...
import com.alibaba.nacos.api.NacosExtension;
...
public class ConfigFilterChainManager implements IConfigFilterChain {
    ...
    public ConfigFilterChainManager(Properties properties) {
        ServiceLoader<NacosExtension> sl = ServiceLoader.load(NacosExtension.class);
        for (NacosExtension configFilterFactory : sl) {
            IConfigFilter filter = configFilterFactory.buildConfigFilter(properties);
            if (filter != null) {
                addFilter(filter);
            }
        }
