...
public class CacheData {
    ...
    static {
        ...
        if (StringUtils.isNotBlank(notifyTimeouts) && NumberUtils.isDigits(notifyTimeouts)) {
            ...
            LOGGER.info(
                    "config listener notify warn timeout millis is set to {}", notifyWarnTimeout);
        } else {
            LOGGER.info(
                    "config listener notify warn timeout millis use default {} millis ",
                    notifyWarnTimeout);
        }
    }
    ...
    /**
     * notify change flag,for notify&sync concurrent control. 1.reset to false if starting to sync
     * with server. 2.update to true if receive config change notification.
     */
    ...
    public void removeListener(Listener listener) {
        ...
        if (listeners.remove(wrap)) {
            LOGGER.info(
                    "[{}] [remove-listener] ok, dataId={}, group={},tenant={}, cnt={}",
                    envName,
                    dataId,
                    group,
                    tenant,
    ...
    class LongNotifyHandler implements Runnable {
        ...
        public void run() {
            ...
            LOGGER.warn(
                    "[{}] [notify-block-monitor] dataId={}, group={},tenant={}, md5={}, "
                            + "receiveConfigInfo execute over {} mills，thread trace block : {}",
                    envName,
                    dataId,
                    group,
                    tenant,
                    ...
                    public void run() {
                        ...
                        try {
                            if (listener instanceof AbstractSharedListener) {
                                ...
                                LOGGER.info(
                                        "[{}] [notify-context] dataId={}, group={},tenant={}, md5={}",
                                        envName,
                                        dataId,
                                        group,
                                        tenant,
                            ...
                            // Before executing the callback, set the thread classloader to the
                            // classloader of
                            // the specific webapp to avoid exceptions or misuses when calling the
                            // spi interface in
                            ...
                            if (listener instanceof AbstractConfigChangeListener) {
                                Map<String, ConfigChangeItem> data =
                                        ConfigChangeHandler.getInstance()
                                                .parseChangeData(
                                                        listenerWrap.lastContent, contentTmp, type);
    ...
    /**
     * 1.first add listener.default is false;need to check. 2.receive config change notify,set
     * false;need to check. 3.last listener is remove,set to false;need to check
     *
     * @return
     */
    ...
    private String loadEncryptedDataKeyFromDiskLocal(
        ...
        return LocalEncryptedDataKeyProcessor.getEncryptDataKeySnapshot(
                envName, dataId, group, tenant);
    }
