...
public class NamingClientProxyDelegateTest {
    ...
    public void testRegisterPersistentServiceByGrpc() throws NacosException, NoSuchFieldException, IllegalAccessException {
        ...
        when(delegate.isAbilitySupportedByServer(AbilityKey.SUPPORT_PERSISTENT_INSTANCE_BY_GRPC)).thenReturn(true);
        delegate.registerService(serviceName, groupName, instance);
        verify(mockGrpcClient, times(1)).registerService(serviceName, groupName, instance);
    }
    ...
    @Test
    public void testIsAbilitySupportedByServer() throws IllegalAccessException, NacosException, NoSuchFieldException {
        String ns = "ns1";
        ServiceInfoHolder holder = Mockito.mock(ServiceInfoHolder.class);
        Properties props = new Properties();
        props.setProperty("serverAddr", "localhost");
        ...
        Assert.assertFalse(delegate.isAbilitySupportedByServer(AbilityKey.SUPPORT_PERSISTENT_INSTANCE_BY_GRPC));

        // when grpc client support register persistent instance
        Mockito.when(mockGrpcClient.isAbilitySupportedByServer(AbilityKey.SUPPORT_PERSISTENT_INSTANCE_BY_GRPC)).thenReturn(true);
        Assert.assertTrue(delegate.isAbilitySupportedByServer(AbilityKey.SUPPORT_PERSISTENT_INSTANCE_BY_GRPC));
    }
