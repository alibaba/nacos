...
public abstract class AbstractHttpClientFactory implements HttpClientFactory {
        ...
        try {
            ioreactor =
                    new DefaultConnectingIOReactor(
                            getIoReactorConfig(), new NameThreadFactory(threadName));
        } catch (IOReactorException e) {
            assignLogger()
                    .error(
                            "[NHttpClientConnectionManager] Create DefaultConnectingIOReactor failed",
                            e);
            throw new IllegalStateException();
        }

        // if the handle return true, then the exception thrown by IOReactor will be ignore, and
        // will not finish the IOReactor.
        ioreactor.setExceptionHandler(
                new IOReactorExceptionHandler() {
                    ...
                    public boolean handle(IOException ex) {
                        assignLogger()
                                .warn(
                                        "[NHttpClientConnectionManager] handle IOException, ignore it.",
                                        ex);
                        return true;
                    }
                    ...
                    public boolean handle(RuntimeException ex) {
                        assignLogger()
                                .warn(
                                        "[NHttpClientConnectionManager] handle RuntimeException, ignore it.",
                                        ex);
                        return true;
                    }
                });
    ...
    /**
     * create the {@link NHttpClientConnectionManager}, the code mainly from {@link
     * HttpAsyncClientBuilder#build()}. we add the {@link IOReactorExceptionHandler} to handle the
     * {@link IOException} and {@link RuntimeException} thrown by the {@link
     * org.apache.http.impl.nio.reactor.BaseIOReactor} when process the event of Network. Using this
     * way to avoid the {@link DefaultConnectingIOReactor} killed by unknown error of network.
     *
     * @param originalRequestConfig request config.
     * @param ioreactor I/O reactor.
     * @return {@link NHttpClientConnectionManager}.
     */
    private NHttpClientConnectionManager getConnectionManager(
            HttpClientConfig originalRequestConfig, DefaultConnectingIOReactor ioreactor) {
    ...
    protected IOReactorConfig getIoReactorConfig() {
        ...
        return IOReactorConfig.custom()
                .setIoThreadCount(httpClientConfig.getIoThreadCount())
                .build();
    }
    ...
    protected void initTls(
            BiConsumer<SSLContext, HostnameVerifier> initTlsBiFunc,
