...
public class NotifyCenter {
    ...
    /**
     * Register a Subscriber. If the Publisher concerned by the Subscriber does not exist, then
     * PublihserMap will preempt a placeholder Publisher with default EventPublisherFactory first.
     *
    ...
    /**
     * Register a Subscriber. If the Publisher concerned by the Subscriber does not exist, then
     * PublihserMap will preempt a placeholder Publisher with specified EventPublisherFactory first.
     *
     * @param consumer subscriber
    ...
    public static void registerSubscriber(
        ...
        if (consumer instanceof SmartSubscriber) {
            for (Class<? extends Event> subscribeType :
    ...
    private static void addSubscriber(
        ...
        synchronized (NotifyCenter.class) {
            ...
            MapUtil.computeIfAbsent(
                    INSTANCE.publisherMap, topic, factory, subscribeType, ringBufferSize);
        }
    ...
    private static boolean removeSubscriber(
            final Subscriber consumer, Class<? extends Event> subscribeType) {
    ...
    /**
     * Request publisher publish event Publishers load lazily, calling publisher. Start () only when
     * the event is actually published.
     *
     ...
     */
    public static EventPublisher registerToPublisher(
     ...
     */
    public static void registerToPublisher(
            final Class<? extends Event> eventType, final EventPublisher publisher) {
        if (null == publisher) {
            return;
        }
        final String topic = ClassUtils.getCanonicalName(eventType);
