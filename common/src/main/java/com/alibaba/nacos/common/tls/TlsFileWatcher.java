 ...
 *
 * <p>Considering that the current client needs to support jdk 1.6 and module dependencies , the
 * WatchFileCenter in the core module is not used
 *
 * @author wangwei
 */
            ...
            if (filePath != null && new File(filePath).exists()) {
                ...
                try {
                    ...
                    fileMd5Map.put(
                            filePath,
                            MD5Utils.md5Hex(
                                    IoUtils.toString(in, Constants.ENCODE), Constants.ENCODE));
                } finally {
    ...
    public void start() {
        if (started.compareAndSet(false, true)) {
            service.scheduleAtFixedRate(
                    () -> {
                        for (Map.Entry<String, FileChangeListener> item :
                                watchFilesMap.entrySet()) {
                            ...
                            try {
                                ...
                                newHash =
                                        MD5Utils.md5Hex(
                                                IoUtils.toString(in, Constants.ENCODE),
                                                Constants.ENCODE);
                            } catch (Exception exception) {
                                LOGGER.warn(
                                        " service has exception when calculate the file MD5. "
                                                + exception);
                                continue;
                            } finally {
                                IoUtils.closeQuietly(in);
                            ...
                            if (!newHash.equals(fileMd5Map.get(filePath))) {
                                LOGGER.info(
                                        filePath + " file hash changed, need reload ssl context");
