...
public class TlsFileWatcherTest {
    ...
    public static void setUpBeforeClass() throws NoSuchFieldException, IllegalAccessException {
        ...
        modifiersField1.setInt(
                watchFilesMapField, watchFilesMapField.getModifiers() & ~Modifier.FINAL);
    ...
    public void testAddFileChangeListener1() throws IOException, IllegalAccessException {
        TlsFileWatcher.getInstance().addFileChangeListener(filePath -> {}, "not/exist/path");

        Assert.assertTrue(
                ((Map<?, ?>) watchFilesMapField.get(TlsFileWatcher.getInstance())).isEmpty());
        Assert.assertTrue(
                ((Map<?, ?>) fileMd5MapField.get(TlsFileWatcher.getInstance())).isEmpty());
    }

    @Test
    public void testAddFileChangeListener2() throws IOException, IllegalAccessException {
        TlsFileWatcher.getInstance().addFileChangeListener(filePath -> {}, (String) null);
    ...
    public void testAddFileChangeListener3() throws IOException, IllegalAccessException {
        TlsFileWatcher.getInstance().addFileChangeListener(filePath -> {}, tempFile.getPath());

        Assert.assertEquals(
                1, ((Map<?, ?>) watchFilesMapField.get(TlsFileWatcher.getInstance())).size());
        Assert.assertEquals(
                1, ((Map<?, ?>) fileMd5MapField.get(TlsFileWatcher.getInstance())).size());
    }
    ...
    public void testStartGivenTlsFileNotChangeThenNoNotify()
        ...
        TlsFileWatcher.getInstance()
                .addFileChangeListener(filePath -> notified.set(true), tempFile.getPath());
    ...
    @Test
    public void testStartGivenTlsFileChangeThenNotifyTheChangeFilePath()
        ...
        TlsFileWatcher.getInstance()
                .addFileChangeListener(
                        ...
                        tempFile.getPath());
        ((Map<String, String>) fileMd5MapField.get(TlsFileWatcher.getInstance()))
                .put("test.txt", "");
