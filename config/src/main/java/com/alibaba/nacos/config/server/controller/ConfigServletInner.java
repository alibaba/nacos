...
public class ConfigServletInner {
    ...
    public String doPollingConfig(
        ...
        if (LongPollingService.isSupportLongPolling(request)) {
            longPollingService.addLongPollingClient(
                    request, response, clientMd5Map, probeRequestSize);
    ...
    public String doGetConfig(
        ...
        return doGetConfig(
                request, response, dataId, group, tenant, tag, isNotify, clientIp, false);
    }
    ...
    public String doGetConfig(
        ...
        boolean isSli = false;
        if (lockResult > 0) {
            // LockResult > 0 means cacheItem is not null and other thread can`t delete this
            // cacheItem
            try {
                ...
                final String configType =
                        (null != cacheItem.getType())
                                ? cacheItem.getType()
                                : FileTypeEnum.TEXT.getFileType();
                response.setHeader("Config-Type", configType);
                FileTypeEnum fileTypeEnum =
                        FileTypeEnum.getFileTypeEnumByFileExtensionOrFileType(configType);
                    ...
                    if (StringUtils.isBlank(tag)) {
                        if (isUseTag(cacheItem, autoTag)) {
                            if (cacheItem.getConfigCacheTags() != null) {
                                ConfigCache configCacheTag =
                                        cacheItem.getConfigCacheTags().get(autoTag);
                            ...
                            response.setHeader(
                                    com.alibaba.nacos.api.common.Constants.VIPSERVER_TAG,
                                    URLEncoder.encode(
                                            autoTag, StandardCharsets.UTF_8.displayName()));
                        } else {
                ...
                putEncryptedDataKeyHeader(
                        response, tag, clientIp, acceptCharset, cacheItem, isBeta, autoTag);

                if (PropertyUtil.isDirectRead()) {
                    Pair<String, String> pair =
                            EncryptionHandler.decryptHandler(
                                    dataId,
                                    configInfoBase.getEncryptedDataKey(),
                                    configInfoBase.getContent());
                ...
                LogUtil.PULL_CHECK_LOG.warn(
                        "{}|{}|{}|{}", groupKey, requestIp, md5, TimeUtils.getCurrentTimeStr());
    ...
    private void putEncryptedDataKeyHeader(
            HttpServletResponse response,
            String tag,
            String clientIp,
            String charset,
