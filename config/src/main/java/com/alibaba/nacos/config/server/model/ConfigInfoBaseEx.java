...
/**
 * ConfigInfoBaseEx. And can't add field, to compatible with old interface(If adding a field, then
 * it will occur compatibility problems).
 *
 * @author Nacos
 */
public class ConfigInfoBaseEx extends ConfigInfoBase {
    ...
    public ConfigInfoBaseEx(
            String dataId, String group, String content, int status, String message) {
        super(dataId, group, content);
        this.status = status;
    ...
    public String toString() {
        return "ConfigInfoBaseEx [status="
                + status
                + ", message="
                + message
                + ", dataId="
