...
public class ConfigSubService {
    ...
    private List<SampleResult> runCollectionJob(
            String url,
        ...
        for (Member ip : ipList) {
            try {
                ...
                LogUtil.DEFAULT_LOG.warn(
                        "Get client info from {} with exception: {} during submit job",
                        ip,
                        e.getMessage());
            }
        }
        ...
        for (Member member : ipList) {
            try {
                ...
                try {
                    ...
                    f.cancel(true);
                    LogUtil.DEFAULT_LOG.warn(
                            "get task result with TimeoutException: {} ", e.getMessage());
                }
            } catch (InterruptedException e) {
                LogUtil.DEFAULT_LOG.warn(
                        "get task result with InterruptedException: {} ", e.getMessage());
            } catch (ExecutionException e) {
                LogUtil.DEFAULT_LOG.warn(
                        "get task result with ExecutionException: {} ", e.getMessage());
            }
        }
     ...
     * @return SampleResult.
     */
    public SampleResult mergeSampleResult(
            SampleResult sampleCollectResult, List<SampleResult> sampleResults) {
            ...
            try {
                ...
                if (result.ok()) {
                    ...
                    LogUtil.DEFAULT_LOG.info(
                            "Can not get clientInfo from {} with {}", ip, result.getData());
                    return null;
                }
            } catch (Exception e) {
                LogUtil.DEFAULT_LOG.warn(
                        "Get client info from {} with exception: {}", ip, e.getMessage());
                return null;
            }
        }
    }

    public SampleResult getCollectSampleResult(
            String dataId, String group, String tenant, int sampleTime) throws Exception {
        ...
        for (int i = 0; i < sampleTime; i++) {
            List<SampleResult> sampleResults =
                    runCollectionJob(url, params, completionService, resultList);
