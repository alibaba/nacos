    ...
    private void autoExpansion(String group, String tenant) {
        ...
        if (initialExpansionPercent > 0) {
            ...
            if (tenant != null) {
                ...
                LogUtil.DEFAULT_LOG.warn("[capacityManagement] The usage({}) already reach the upper limit({}) when init the tenant({}), automatic upgrade to ({})",
                        usage, defaultQuota, tenant, finalQuota);
            } else {
                groupCapacityPersistService.updateQuota(group, finalQuota);
                LogUtil.DEFAULT_LOG.warn("[capacityManagement] The usage({}) already reach the upper limit({}) when init the group({}), automatic upgrade to ({})",
