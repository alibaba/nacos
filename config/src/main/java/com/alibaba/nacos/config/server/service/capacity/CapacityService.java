...
public class CapacityService {
    ...
    public void init() {
        ...
        ConfigExecutor.scheduleCorrectUsageTask(
                () -> {
                    ...
                    watch.stop();
                    LOGGER.info(
                            "[capacityManagement] end correct usage, cost: {}s",
    ...
    private void correctGroupUsage() {
        ...
        while (true) {
            List<GroupCapacity> groupCapacityList =
                    groupCapacityPersistService.getCapacityList4CorrectUsage(lastId, pageSize);
    ...
    private void correctTenantUsage() {
        ...
        while (true) {
            List<TenantCapacity> tenantCapacityList =
                    tenantCapacityPersistService.getCapacityList4CorrectUsage(lastId, pageSize);
    ...
    /**
     * To Cluster. 1.If the capacity information does not exist, initialize the capacity
     * information. 2.Update capacity usage, plus or minus one.
     *
    ...
    public boolean insertAndUpdateClusterUsage(CounterMode counterMode, boolean ignoreQuotaLimit) {
        ...
        return updateGroupUsage(
                counterMode,
                GroupCapacityPersistService.CLUSTER,
     ...
     * 1.If the capacity information does not exist, initialize the capacity information. 2.Update
     * capacity usage, plus or minus one.
     *
     * @param counterMode increase or decrease mode.
     ...
     */
    public boolean insertAndUpdateGroupUsage(
            CounterMode counterMode, String group, boolean ignoreQuotaLimit) {
        GroupCapacity groupCapacity = getGroupCapacity(group);
        if (groupCapacity == null) {
            initGroupCapacity(group, null, null, null, null);
        }
        return updateGroupUsage(
                counterMode, group, PropertyUtil.getDefaultGroupQuota(), ignoreQuotaLimit);
    }
    ...
    private boolean updateGroupUsage(
            CounterMode counterMode, String group, int defaultQuota, boolean ignoreQuotaLimit) {
        ...
        if (CounterMode.INCREMENT == counterMode) {
            ...
            // First update the quota according to the default value. In most cases, it is the
            // default value.
    ...
    /**
     * Initialize the capacity information of the group. If the quota is reached, the capacity will
     * be automatically expanded to reduce the operation and maintenance cost.
     *
     ...
     * @return init result.
     */
    private boolean initGroupCapacity(
            String group,
            Integer quota,
            Integer maxSize,
    ...
    private boolean insertGroupCapacity(
            String group,
            Integer quota,
            Integer maxSize,
            Integer maxAggrCount,
        ...
        try {
            ...
            // In order to update the default quota, only the Nacos configuration needs to be
            // modified,
            ...
            // In order to update the default size, you only need to modify the Nacos configuration
            // without updating most of the data in the table.
     ...
     * @return operate successfully or not.
     */
    public boolean insertAndUpdateTenantUsage(
            CounterMode counterMode, String tenant, boolean ignoreQuotaLimit) {
    ...
    private boolean updateTenantUsage(
            CounterMode counterMode, String tenant, boolean ignoreQuotaLimit) {
        final Timestamp now = TimeUtils.getCurrentTime();
    ...
    /**
     * Initialize the capacity information of the tenant. If the quota is reached, the capacity will
     * be automatically expanded to reduce the operation and maintenance cos.
     *
     * @param tenant tenant string value.
     * @return init result.
     */
     ...
     * @return
     */
    public boolean initTenantCapacity(
            String tenant,
            Integer quota,
            Integer maxSize,
            Integer maxAggrCount,
    ...
    private boolean insertTenantCapacity(
            String tenant,
            Integer quota,
            Integer maxSize,
            Integer maxAggrCount,
    ...
    /**
     * Support for API interface, Tenant: initialize if the record does not exist, and update the
     * capacity quota or content size directly if it exists.
     *
     * @param group group string value.
     * @param tenant tenant string value.
     * @param quota quota int value.
     ...
     * @return operate successfully or not.
     */
    public boolean insertOrUpdateCapacity(
            String group,
            String tenant,
            Integer quota,
            Integer maxSize,
        ...
        if (StringUtils.isNotBlank(tenant)) {
            ...
            return tenantCapacityPersistService.updateTenantCapacity(
                    tenant, quota, maxSize, maxAggrCount, maxAggrSize);
        }
        ...
        return groupCapacityPersistService.updateGroupCapacity(
                group, quota, maxSize, maxAggrCount, maxAggrSize);
    }
}
