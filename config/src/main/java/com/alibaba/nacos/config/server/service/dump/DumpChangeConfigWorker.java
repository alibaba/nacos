    ...
    public void run() {
        ...
        try {
            ...
            LogUtil.DEFAULT_LOG.info(
                    "DumpChange start ,from time {},current time {}", startTime, currentTime);
            ...
            while (true) {
                List<ConfigInfoWrapper> configDeleted =
                        historyConfigInfoPersistService.findDeletedConfig(
                                startTime, deleteCursorId, pageSize);
            ...
            LogUtil.DEFAULT_LOG.info(
                    "Check delete configs finished,cost:{}",
            ...
            while (true) {
                LogUtil.DEFAULT_LOG.info(
                        "Check changed configs from  time {},lastMaxId={}",
                        startTime,
                        changeCursorId);
                List<ConfigInfoWrapper> changeConfigs =
                        configInfoPersistService.findChangeConfig(
                                startTime, changeCursorId, pageSize);
                for (ConfigInfoWrapper cf : changeConfigs) {
                    ConfigCacheService.dumpChange(
                            cf.getDataId(),
                            cf.getGroup(),
                            cf.getTenant(),
                            cf.getContent(),
                    ...
                    LogUtil.DEFAULT_LOG.info(
                            "[dump-change-check-ok] {}, {}, length={}, md5={}",
                            new Object[] {
            ...
            LogUtil.DEFAULT_LOG.info(
                    "Check changed configs finished,cost:{},set next start time to {}",
