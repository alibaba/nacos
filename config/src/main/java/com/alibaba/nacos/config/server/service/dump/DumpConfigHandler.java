...
public class DumpConfigHandler extends Subscriber<ConfigDumpEvent> {
    ...
    public static boolean configDump(ConfigDumpEvent event) {
        ...
        if (StringUtils.isNotBlank(event.getGrayName())) {
            ...
            if (!event.isRemove()) {
                result = ConfigCacheService.dumpGray(dataId, group, namespaceId,
                        event.getGrayName(),
                ...
                result = ConfigCacheService.removeGray(dataId, group, namespaceId,
                        event.getGrayName());
                if (result) {
                    ConfigTraceService.logDumpGrayNameEvent(dataId, group, namespaceId,
                            event.getGrayName(), null,
                            lastModified, event.getHandleIp(),
