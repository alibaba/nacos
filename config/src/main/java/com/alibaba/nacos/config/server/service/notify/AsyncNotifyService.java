...
public class AsyncNotifyService {
                ...
                if (memberManager.getSelf().equals(member)) {
                    if (syncRequest.isBeta()) {
                        dumpService.dumpBeta(
                                syncRequest.getDataId(),
                                syncRequest.getGroup(),
                                syncRequest.getTenant(),
                                syncRequest.getLastModified(),
                                NetUtils.localIP());
                ...
                if (memberManager.hasMember(member.getAddress())) {
                    ...
                    if (unHealthNeedDelay) {
                        ...
                        ConfigTraceService.logNotifyEvent(
                                ...
                                InetUtils.getSelfIP(),
                                event,
                                ConfigTraceService.NOTIFY_TYPE_UNHEALTH,
                                0,
                                member.getAddress());
    ...
    class AsyncRpcNotifyCallBack implements RequestCallBack<ConfigChangeClusterSyncResponse> {
        ...
        public void onResponse(ConfigChangeClusterSyncResponse response) {
            ...
            if (response.isSuccess()) {
                ...
                LOGGER.error(
                        "[notify-error] target:{} dataId:{} group:{} ts:{} code:{}",
                        task.member.getAddress(),
                        task.getDataId(),
                        task.getGroup(),
                        task.getLastModified(),
        ...
        public void onException(Throwable ex) {
            ...
            long delayed = System.currentTimeMillis() - task.getLastModified();
            LOGGER.error(
                    "[notify-exception] target:{} dataId:{} group:{} ts:{} ex:{}",
                    task.member.getAddress(),
                    task.getDataId(),
                    task.getGroup(),
                    task.getLastModified(),
    ...
    /**
     * get delayTime and also set failCount to task; The failure time index increases, so as not to
     * retry invalid tasks in the offline scene, which affects the normal synchronization.
     *
