...
public class CommunicationControllerTest {
    ...
    @InjectMocks CommunicationController communicationController;

    private MockMvc mockMvc;

    @Mock private ServletContext servletContext;

    @Mock DumpService dumpService;

    @Mock LongPollingService longPollingService;

    @Mock ConfigChangeListenContext configChangeListenContext;
    ...
    public void setUp() {
        ...
        ReflectionTestUtils.setField(
                communicationController, "longPollingService", longPollingService);
        ReflectionTestUtils.setField(
                communicationController, "configChangeListenContext", configChangeListenContext);
        ReflectionTestUtils.setField(
                communicationController, "connectionManager", connectionManager);
    ...
    public void testNotifyConfigInfo() throws Exception {
        ...
        MockHttpServletRequestBuilder builder =
                MockMvcRequestBuilders.get(Constants.COMMUNICATION_CONTROLLER_PATH + "/dataChange")
                        .param("dataId", "test")
                        .param("group", "test");
        String actualValue =
    ...
    public void testGetSubClientConfig1x() throws Exception {
        ...
        when(longPollingService.getCollectSubscribleInfo("test", "test", "test"))
                .thenReturn(result);

        MockHttpServletRequestBuilder builder =
                MockMvcRequestBuilders.get(
                                Constants.COMMUNICATION_CONTROLLER_PATH + "/configWatchers")
                        .param("dataId", "test")
                        .param("group", "test")
                        .param("tenant", "test");
        String actualValue =
    ...
    public void testGetSubClientConfigByIp() throws Exception {
        ...
        ConnectionMeta connectionMeta =
                new ConnectionMeta(ip, ip, ip, 8888, 9848, "GRPC", "", "", new HashMap<>());
        ...
        MockHttpServletRequestBuilder builder =
                MockMvcRequestBuilders.get(
                                Constants.COMMUNICATION_CONTROLLER_PATH + "/watcherConfigs")
                        .param("ip", ip);
        String actualValue =
                mockMvc.perform(builder).andReturn().getResponse().getContentAsString();
        Assert.assertEquals(
