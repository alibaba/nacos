...
public class GroupCapacityPersistServiceTest {
    ...
    @InjectMocks private GroupCapacityPersistService service;

    @Mock private JdbcTemplate jdbcTemplate;

    @Mock private DataSourceService dataSourceService;

    @Mock private MapperManager mapperManager;
    ...
    public void setUp() {
        ...
        doReturn(new GroupCapacityMapperByMysql())
                .when(mapperManager)
    ...
    public void testInsertGroupCapacity() {
        Mockito.when(
                        jdbcTemplate.update(
                                ...
                                argThat(
                                        (ArgumentMatcher<GeneratedKeyHolder>)
                                                keyHolder -> {
    ...
    public void testGetClusterUsage() {
        doReturn(new ConfigInfoMapperByMySql())
                .when(mapperManager)
                .findMapper(any(), eq(TableConstant.CONFIG_INFO));
    ...
    public void testGetCapacityList4CorrectUsage() {
        ...
        when(jdbcTemplate.query(
                        anyString(), eq(new Object[] {lastId, pageSize}), any(RowMapper.class)))
                .thenReturn(list);
