...
public class GroupCapacityPersistServiceTest {
    ...
    @Spy
    private GroupCapacityPersistService service = Mockito.spy(GroupCapacityPersistService.class);
    ...
    @Spy
    private MapperManager mapperManager = Mockito.spy(MapperManager.instance(false));
    ...
    public void testInsertGroupCapacity() {
        Mockito.when(jdbcTemplate.update(any(PreparedStatementCreator.class), argThat(
    ...
    public void testGetClusterUsage() {
        doReturn(new ConfigInfoMapperByMySql())
            .when(mapperManager).findMapper(any(), eq(TableConstant.CONFIG_INFO));
