...
class EmbeddedConfigInfoTagPersistServiceImplTest {
    ...
    void testInsertOrUpdateTagOfAdd() {
        ...
        ConfigOperateResult configOperateResult = embeddedConfigInfoTagPersistService
                .insertOrUpdateTag(configInfo, tag, srcIp, srcUser);
        ...
        embeddedStorageContextHolderMockedStatic.verify(
                () -> EmbeddedStorageContextHolder.addSqlContext(anyString(), eq(dataId), eq(group),
                        ...
                        eq(content), eq(MD5Utils.md5Hex(content, Constants.PERSIST_ENCODE)),
    ...
    void testInsertOrUpdateTagOfUpdate() {
        ...
        embeddedStorageContextHolderMockedStatic
                .verify(() -> EmbeddedStorageContextHolder.addSqlContext(anyString(), eq(content),
                        ...
                        eq(srcUser), any(Timestamp.class), eq(appName),
    ...
    void testInsertOrUpdateTagCasOfUpdate() {
        ...
        embeddedStorageContextHolderMockedStatic.verify(
                () -> EmbeddedStorageContextHolder.addSqlContext(anyString(), eq(content),
                        eq(MD5Utils.md5Hex(content, Constants.PERSIST_ENCODE)), eq(srcIp),
                        eq(srcUser), any(Timestamp.class), eq(appName),
                        eq(dataId), eq(group), eq(tenant), eq(tag), eq(configInfo.getMd5())),
                times(1));
        assertEquals(configInfoStateWrapper.getId(), configOperateResult.getId());
        assertEquals(configInfoStateWrapper.getLastModified(),
                configOperateResult.getLastModified());
    ...
    void testFindAllConfigInfoTagForDumpAll() {
        ...
        //mock query list
        Mockito.when(databaseOperate.queryMany(anyString(), eq(new Object[] {}),
                eq(CONFIG_INFO_TAG_WRAPPER_ROW_MAPPER)))
                .thenReturn(mockTagList);
        int pageNo = 3;
        int pageSize = 100;
        //execute & verify
        Page<ConfigInfoTagWrapper> returnTagPage = embeddedConfigInfoTagPersistService
                .findAllConfigInfoTagForDumpAll(pageNo, pageSize);
    ...
    void testFindConfigInfoTags() {
        ...
        Mockito.when(databaseOperate.queryMany(anyString(),
                eq(new Object[] {dataId, group, tenant}), eq(String.class)))
