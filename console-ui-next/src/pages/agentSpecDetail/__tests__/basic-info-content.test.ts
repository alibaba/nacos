...
describe('AgentSpec detail basic info content', () => {
  it('uses the current version lifecycle status for the status field', () => {
    ...
    expect(DETAIL_SOURCE).toContain("const currentVersionCreateTime = currentVersionSummary?.createTime || 0;");
    expect(DETAIL_SOURCE).toContain("t('agentSpec.createTime')");
  });

  it('guards the fallback detail reload so the page does not loop on missing version responses', () => {
    expect(DETAIL_SOURCE).toContain('const autoLoadedVersionRef = useRef<string | null>(null);');
    expect(DETAIL_SOURCE).toContain('autoLoadedVersionRef.current = null;');
    expect(DETAIL_SOURCE).toContain('if (fallbackVersion && autoLoadedVersionRef.current !== fallbackVersion) {');
    expect(DETAIL_SOURCE).toContain('autoLoadedVersionRef.current = fallbackVersion;');
    expect(DETAIL_SOURCE).toContain('loadDetail(fallbackVersion);');
  });
  ...
  it('falls back to getVersion for resource content when detail.agentSpec is absent', () => {
    ...
    expect(DETAIL_SOURCE).toContain('const spec = detail.agentSpec ?? detailDocument ?? {');
  });

  it('renders the status field above the create time field in the basic info card', () => {
    expect(DETAIL_SOURCE.indexOf("label={t('agentSpec.status')}")).toBeLessThan(
      DETAIL_SOURCE.indexOf("label={t('agentSpec.createTime')}"),
    );
  });
  ...
  it('uses the revised online-version copy in Chinese locale', () => {
    expect(ZH_LOCALE_SOURCE).toContain('"onlineCount": "{{count}} 个版本已上线"');
    expect(DETAIL_SOURCE).toContain("const onlineVersionCountLabel = t('agentSpec.onlineCount', { count: detail.onlineCnt ?? 0 });");
  });

  it('renders labels as read-only content and sizes the resource viewer responsively', () => {
    expect(DETAIL_SOURCE).not.toContain('<LabelEditor');
    expect(DETAIL_SOURCE).toContain("const labelEntries = Object.entries(detail.labels || {}).filter(([key]) => key !== 'latest');");
    expect(DETAIL_SOURCE).toContain('className="flex h-[calc(100vh-88px)] min-h-0 flex-col gap-5 overflow-hidden"');
    expect(DETAIL_SOURCE).toContain('className="flex min-h-0 flex-1 flex-col overflow-hidden py-0 gap-0"');
    expect(DETAIL_SOURCE).toContain('className="h-full min-h-0"');
    expect(DETAIL_SOURCE).toContain("t('agentSpec.noLabels')");
  });
