...
import { getValidActionsWithContext, sortVersionsDescending } from './prompt-version-utils';
...
export function PromptVersionTimeline({
  ...
  const actionMeta: Record<string, { icon: React.ReactNode; labelKey: string; variant?: 'default' | 'outline' | 'destructive' | 'ghost' }> = {
    submit: { icon: <Send className="h-3 w-3" />, labelKey: 'prompt.submit' },
    publish: { icon: <Rocket className="h-3 w-3" />, labelKey: 'prompt.publish' },
    forcePublish: { icon: <ShieldAlert className="h-3 w-3" />, labelKey: 'prompt.forcePublish', variant: 'outline' },
    online: { icon: <Globe className="h-3 w-3" />, labelKey: 'prompt.online' },
    offline: { icon: <PowerOff className="h-3 w-3" />, labelKey: 'prompt.offline', variant: 'outline' },
    deleteDraft: { icon: <Trash2 className="h-3 w-3" />, labelKey: 'common.delete', variant: 'destructive' },
    createDraftFrom: { icon: <GitBranch className="h-3 w-3" />, labelKey: 'prompt.createDraftFrom' },
  };
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
            ...
            <div key={v.version} className="relative flex gap-3 pb-4">
              ...
              >
                ...
                {allLabels && (() => {
                  ...
                  return labelKeys.length > 0 ? (
                    <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                      ...
                      {labelKeys.map((key) => (
                        <Badge key={key} variant="outline" className="text-[10px] px-1.5 py-0 h-4 font-mono">
                          {key}
                        </Badge>
                      ))}
                    </div>
                ...
                <div className="flex items-center gap-3 mt-1 text-[11px] text-muted-foreground">
                  {v.srcUser && <span>{v.srcUser}</span>}
                  <span className="inline-flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    {dayjs(v.gmtModified).format('YYYY-MM-DD HH:mm')}
                  </span>
                </div>

                {v.commitMsg && (
                  <p className="mt-1 text-xs text-muted-foreground line-clamp-2">{v.commitMsg}</p>
                )}

                {/* Action buttons */}
                {(actionItems.length > 0 || onSaveLabels) && (
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap" onClick={(e) => e.stopPropagation()}>
                    {actionItems.map((item) => {
                      const meta = actionMeta[item.action];
                      if (!meta) return null;

                      const handler = item.action === 'deleteDraft' ? onDeleteDraft : actionHandlers[item.action];
                    ...
                    {onSaveLabels && v.status !== 'draft' && v.status !== 'reviewing' && (
                      <Button variant="ghost" size="sm" className="h-6 px-2 text-[11px]" onClick={() => setLabelEditVersion(v.version)}>
        ...
        {sorted.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">{t('prompt.noVersions')}</p>
        )}
      </div>
      ...
      <Dialog open={!!forcePublishConfirmVersion} onOpenChange={(open) => !open && setForcePublishConfirmVersion(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('prompt.forcePublishConfirmDesc', { version: forcePublishConfirmVersion ?? '' })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => {
                if (forcePublishConfirmVersion) onForcePublish?.(forcePublishConfirmVersion);
