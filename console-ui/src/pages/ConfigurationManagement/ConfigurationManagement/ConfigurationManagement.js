...
class ConfigurationManagement extends React.Component {
  ...
  navTo(url, record) {
    ...
    this.tenant = getParams('namespace') || 'public'; // Save tenant parameter for current instance
    switch (url) {
  ...
  goDetail(record) {
    this.serverId = getParams('serverId') || 'center';
    this.tenant = getParams('namespace') || 'public'; // Save tenant parameter for current instance
    // When clicking details to another page, preserve original search conditions when returning. For example: record.dataId is for details, this.dataId is for search conditions.
    this.props.history.push(
