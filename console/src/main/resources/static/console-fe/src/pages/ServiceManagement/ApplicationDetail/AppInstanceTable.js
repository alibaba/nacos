...
import { HEALTHY_COLOR_MAPPING } from '../ServiceDetail/constant';
import EditInstanceDialog from '../ServiceDetail/EditInstanceDialog';

@ConfigProvider.config
class AppInstanceTable extends React.Component {
  ...
  getInstanceList() {
    const { namespaceId, ip, port } = this.props;
    const { pageSize, pageNum } = this.state;
    const startIndex = (pageNum - 1) * pageSize;
    const endIndex = pageNum * pageSize - 1;
    request({
      url: 'v1/ns/catalog/applications/instances',
      data: {
        namespaceId,
        ip,
        port,
        ...
        if (instanceList.length > 0) {
          this.setState({
            instance: {
              ...
              list: instanceList.slice(startIndex, endIndex),
            },
          });
        }
      },
  ...
  switchState(index, record) {
    ...
    const { clusterName, serviceName } = record;
    request({
      method: 'PUT',
      url: 'v1/ns/instance',
      data: {
  ...
  render() {
    ...
    const { instance, pageSize, loading, serviceName, clusterName } = this.state;
