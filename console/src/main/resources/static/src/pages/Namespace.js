                ...
                if (res.code === 200) {
                    ...
                    Dialog.alert({
                        ...
                        <div style={{ marginTop: '10px' }}>
                            ...
                            <p>
                                <span style={{ color: '#999', marginRight: 5 }}>{aliwareIntl.get('nacos.page.namespace.ecs_ram_role')}</span>
                                <a href={window._getLink && window._getLink("ecsInstanceRamRolesUse")} target="_blank">{aliwareIntl.get('nacos.page.namespace._Details_of6')}</a>
                            </p>
                            <p>
                                <span style={{ color: '#999', marginRight: 5 }}>{aliwareIntl.get('nacos.page.namespace.AccessKey_recommended1')}</span>
                                <span style={{ color: '#c7254e' }}>
                                    <a href={window._getLink && window._getLink("getAk")} target="_blank">{aliwareIntl.get('nacos.page.namespace.click_on_the_obtain_of1')}</a>
                                </span>
                            </p>
                            <p>
                                <span style={{ color: '#999', marginRight: 5 }}>{aliwareIntl.get('nacos.page.namespace.SecretKey_recommended3')}</span>
                                <span style={{ color: '#c7254e' }}>
                                    <a href={window._getLink && window._getLink("getAk")} target="_blank">{aliwareIntl.get('nacos.page.namespace.click_on_the_obtain_of1')}</a>
                                </span>
                            </p>
                            <p>
                                <span style={{ color: '#999', marginRight: 5 }}>{aliwareIntl.get('nacos.page.namespace.ACM_dedicated_AccessKey_will_the_waste,_does_not_recommend_the_use_of3')}</span>
                                <span style={{ color: '#c7254e' }}>
                                    {obj.accessKey}
                                </span>
                            </p>
                            <p>
                                <span style={{ color: '#999', marginRight: 5 }}>{aliwareIntl.get('nacos.page.namespace.ACM_special_SecretKey_will_be_abandoned,_not_recommended_for_use4')}</span>
                                <span style={{ color: '#c7254e' }}>
                                    {obj.secretKey}
                                </span>
                            </p>
...
//            url: `/diamond-ops/service/serverId/${serverId}/namespaceInfo`,
    ...
    renderOption(value, index, record) {
        let _delinfo = <a href="javascript:;" onClick={this.removeNamespace.bind(this, record)} style={{ marginRight: 10 }}>{aliwareIntl.get('com.alibaba.nacos.page.namespace.delete')}</a>;
        ...
        let _detailinfo = <a href="javascript:;" onClick={this.detailNamespace.bind(this, record)} style={{ marginRight: 10 }}>{aliwareIntl.get('nacos.page.namespace.details')}</a>;

        let _editinfo = <a href="javascript:;" onClick={this.openToEdit.bind(this, record)}>{aliwareIntl.get('com.alibaba.nacos.page.namespace.edit')}</a>;
                ...
                <Loading shape="flower" tip="Loading..." color="#333" style={{ width: '100%' }} visible={this.state.loading}>
                    <div>
                        <div style={{ textAlign: 'right', marginBottom: 10 }}>
                            ...
                            <Button type="primary" style={{ marginRight: 20, marginTop: 10 }} onClick={this.addNameSpace.bind(this)}>{aliwareIntl.get('com.alibaba.nacos.page.namespace.add')}</Button>
                        </div>
                        <div>
