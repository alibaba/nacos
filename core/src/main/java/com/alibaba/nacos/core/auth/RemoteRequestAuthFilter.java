...
public class RemoteRequestAuthFilter extends AbstractRequestFilter {
    ...
    public Response filter(Request request, RequestMeta meta, Class handlerClazz) throws NacosException {
        ...
        try {
            ...
            defaultResponseInstance.setErrorInfo(HttpServletResponse.SC_FORBIDDEN, e.getMessage());
            return defaultResponseInstance;
        } catch (IllegalArgumentException e) {
            Response defaultResponseInstance = getDefaultResponseInstance(handlerClazz);
            
            defaultResponseInstance
                    .setErrorInfo(HttpServletResponse.SC_BAD_REQUEST, ExceptionUtil.getAllExceptionMsg(e));
            return defaultResponseInstance;
        } catch (Exception e) {
            Response defaultResponseInstance = getDefaultResponseInstance(handlerClazz);
            defaultResponseInstance
                    .setErrorInfo(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Server failed," + e.getMessage());
            return defaultResponseInstance;
        }
