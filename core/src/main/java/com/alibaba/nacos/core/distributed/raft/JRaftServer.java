 ...
 *
 * <p>Why do we need to create a raft group based on the value of LogProcessor group (), that is,
 * each function module has its own state machine. Because each LogProcessor corresponds to a
 * different functional module, such as Nacos's naming module and config module, these two modules
 * are independent of each other and do not affect each other. If we have only one state machine, it
 * is equal to the log of all functional modules The processing is loaded together. Any module that
 * has an exception during the log processing and a long block operation will affect the normal
 * operation of other functional modules.
 *
...
public class JRaftServer {
    ...
    void init(RaftConfig config) {
        ...
        rpcRequestTimeoutMs =
                ConvertUtils.toInt(
                        raftConfig.getVal(RaftSysConstants.RAFT_RPC_REQUEST_TIMEOUT_MS),
    ...
    synchronized void createMultiRaftGroup(Collection<RequestProcessor4CP> processors) {
        // There is no reason why the LogProcessor cannot be processed because of the
        // synchronization
        if (!this.isStarted) {
        ...
        for (RequestProcessor4CP processor : processors) {
            ...
            doSnapshotInterval =
                    CollectionUtils.isEmpty(processor.loadSnapshotOperate())
                            ? 0
                            : doSnapshotInterval;
            ...
            RaftExecutor.scheduleRaftMemberRefreshJob(
                    ...
                    nodeOptions.getElectionTimeoutMs(),
                    period,
                    TimeUnit.MILLISECONDS);
            multiRaftGroup.put(
                    groupName, new RaftGroupTuple(node, processor, raftGroupService, machine));
        }
    }
    ...
    CompletableFuture<Response> get(final ReadRequest request) {
        ...
        try {
            ...
            MetricsMonitor.raftReadFromLeader();
            Loggers.RAFT.warn(
                    "Raft linear read failed, go to Leader read logic : {}", e.toString());
            // run raft read
            readFromLeader(request, future);
            return future;
        }
    ...
    public void readFromLeader(
            final ReadRequest request, final CompletableFuture<Response> future) {
        commit(request.getGroup(), request, future);
    }

    public CompletableFuture<Response> commit(
            final String group, final Message data, final CompletableFuture<Response> future) {
    ...
    public void applyOperation(Node node, Message data, FailoverClosure closure) {
        ...
        task.setDone(
                new NacosClosure(
                        data,
                        status -> {
                            NacosClosure.NacosStatus nacosStatus =
                                    (NacosClosure.NacosStatus) status;
        ...
        task.setData(
                (ByteBuffer)
                        ByteBuffer.allocate(requestTypeFieldBytes.length + dataBytes.length)
                                .put(requestTypeFieldBytes)
                                .put(dataBytes)
                                .position(0));
        node.apply(task);
    }

    private void invokeToLeader(
            final String group,
            final Message request,
            final int timeoutMillis,
            FailoverClosure closure) {
        try {
            final Endpoint leaderIp =
                    Optional.ofNullable(getLeader(group))
    ...
    boolean peerChange(JRaftMaintainService maintainService, Set<String> newPeers) {
        // This is only dealing with node deletion, the Raft protocol, where the node adds itself to
        // the cluster when it starts up
        ...
        multiRaftGroup.forEach(
                new BiConsumer<String, RaftGroupTuple>() {
                    @Override
    ...
    void refreshRouteTable(String group) {
        ...
        try {
            ...
            String oldLeader =
                    Optional.ofNullable(instance.selectLeader(groupName))
                            .orElse(PeerId.emptyPeer())
                            .getEndpoint()
                            .toString();
            ...
            if (!status.isOk()) {
                Loggers.RAFT.error(
                        "Fail to refresh leader for group : {}, status is : {}", groupName, status);
            }
            status =
                    instance.refreshConfiguration(
                            this.cliClientService, groupName, rpcRequestTimeoutMs);
            if (!status.isOk()) {
                Loggers.RAFT.error(
                        "Fail to refresh route configuration for group : {}, status is : {}",
                        groupName,
                        status);
            }
        } catch (Exception e) {
            Loggers.RAFT.error(
                    "Fail to refresh raft metadata info for group : {}, error is : {}",
                    groupName,
                    e);
        }
    }
