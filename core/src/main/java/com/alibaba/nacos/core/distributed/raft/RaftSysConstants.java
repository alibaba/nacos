...
public final class RaftSysConstants {
    ...
    /**
     * The maximum number of logs sent from the leader to the followers is 1024 by default 从 leader
     * 往 follower 发送的最大日志个数，默认 1024
     */
    public static final int DEFAULT_MAX_ENTRIES_SIZE = 1024;

    /**
     * The maximum body size of the log sent from the leader to the followers is 512K by default 从
     * leader 往 follower 发送日志的最大 body 大小，默认 512K
     */
    ...
    /**
     * The election timer interval will be a random maximum outside the specified time, 1 second by
     * default 选举定时器间隔会在指定时间之外随机的最大范围，默认1秒
     */
    public static final int DEFAULT_MAX_ELECTION_DELAY_MS = 1000;

    /**
     * Specifies the ratio of the election timeout to the heartbeat interval. Heartbeat interval is
     * equal to the electionTimeoutMs/electionHeartbeatFactor, default one of 10 points.
    ...
    /**
     * The tasks submitted to the leader will accumulate one batch into the maximum batch size
     * stored in the log, and 32 tasks will be assigned by default 向 leader
     * 提交的任务累积一个批次刷入日志存储的最大批次大小，默认 32 个任务
     */
    ...
    /**
     * If fsync is called by writing snapshot/raft information, the default is false. If sync is
     * true, it is better to respect sync 写入 snapshot/raft 元信息是否调用 fsync，默认为 false，在 sync 为 true
     * 的情况下，优选尊重 sync
     */
    public static final boolean DEFAULT_SYNC_META = false;

    /**
     * Internal disruptor buffer size, need to be appropriately adjusted for high write throughput
     * applications, default 16384 内部 disruptor buffer 大小，如果是写入吞吐量较高的应用，需要适当调高该值，默认 16384
     */
    public static final int DEFAULT_DISRUPTOR_BUFFER_SIZE = 16384;

    /**
     * Whether to enable replicated pipeline request optimization by default 是否启用复制的 pipeline
     * 请求优化，默认打开
     */
    public static final boolean DEFAULT_REPLICATOR_PIPELINE = true;

    /**
     * Maximum in-flight requests with pipeline requests enabled, 256 by default 在启用 pipeline
     * 请求情况下，最大 in-flight 请求数，默认256
     */
    ...
    /**
     * Election timer interval will be a random maximum outside the specified time, default is 1
     * second
     */
    public static final String MAX_ELECTION_DELAY_MS = "max_election_delay_ms";

    /**
     * Specify the ratio between election timeout and heartbeat interval. Heartbeat interval is
     * equal to electionTimeoutMs/electionHeartbeatFactor，One tenth by default.
     */
    public static final String ELECTION_HEARTBEAT_FACTOR = "election_heartbeat_factor";

    /**
     * The tasks submitted to the leader accumulate the maximum batch size of a batch flush log
     * storage. The default is 32 tasks.
     */
    ...
    /**
     * Whether to write snapshot / raft meta-information to call fsync. The default is false. When
     * sync is true, it is preferred to respect sync.
     */
    public static final String SYNC_META = "sync_meta";

    /**
     * Internal disruptor buffer size. For applications with high write throughput, you need to
     * increase this value. The default value is 16384.
     */
