...
public class DistributedDatabaseOperateImpl extends RequestProcessor4CP
    ...
    public <R> R queryOne(String sql, Class<R> cls) {
        try {
            ...
            byte[] data = serializer.serialize(
                    SelectRequest.builder().queryType(QueryType.QUERY_ONE_NO_MAPPER_NO_ARGS)
                            .sql(sql)
    ...
    public <R> R queryOne(String sql, Object[] args, Class<R> cls) {
        try {
            ...
            byte[] data = serializer.serialize(
                    SelectRequest.builder().queryType(QueryType.QUERY_ONE_NO_MAPPER_WITH_ARGS)
                            .sql(sql).args(args)
                            .className(cls.getCanonicalName()).build());
    ...
    public <R> List<R> queryMany(String sql, Object[] args, Class<R> rClass) {
        try {
            ...
            byte[] data = serializer.serialize(
                    SelectRequest.builder().queryType(QueryType.QUERY_MANY_NO_MAPPER_WITH_ARGS)
                            .sql(sql).args(args)
    ...
    public List<Map<String, Object>> queryMany(String sql, Object[] args) {
        try {
            ...
            byte[] data = serializer.serialize(
                    SelectRequest.builder().queryType(QueryType.QUERY_MANY_WITH_LIST_WITH_ARGS)
                            .sql(sql).args(args)
                            .build());
                    ...
                    if (submit) {
                        ...
                        CompletableFuture<Response> future = protocol.writeAsync(
                                WriteRequest.newBuilder().setGroup(group())
                                        .setData(
                                                ByteString.copyFrom(serializer.serialize(requests)))
        ...
        try {
            ...
            final String key =
                    System.currentTimeMillis() + "-" + group() + "-"
                            ...
                            + MD5Utils.md5Hex(sqlContext.toString(),
                                    PersistenceConstant.DEFAULT_ENCODE);
                ...
                this.protocol.writeAsync(request)
                        .whenComplete((BiConsumer<Response, Throwable>) (response, ex) -> {
                            ...
                            consumer.accept(response.getSuccess(),
                                    StringUtils.isBlank(errMsg) ? null
    ...
    public Response onRequest(final ReadRequest request) {
        ...
        try {
            ...
            ByteString bytes = data == null ? ByteString.EMPTY
            ...
            return Response.newBuilder().setSuccess(false)
                    .setErrMsg(ClassUtils.getSimplaName(e) + ":"
