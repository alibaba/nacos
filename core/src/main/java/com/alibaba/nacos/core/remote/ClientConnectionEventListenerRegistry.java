...
public class ClientConnectionEventListenerRegistry {
    ...
    public void notifyClientConnected(final Connection connection) {
        ...
        for (ClientConnectionEventListener clientConnectionEventListener :
                clientConnectionEventListeners) {
            try {
                clientConnectionEventListener.clientConnected(connection);
            } catch (Throwable throwable) {
                Loggers.REMOTE.info(
                        "[NotifyClientConnected] failed for listener {}",
    ...
    public void notifyClientDisConnected(final Connection connection) {
        ...
        for (ClientConnectionEventListener clientConnectionEventListener :
            ...
            try {
                ...
                Loggers.REMOTE.info(
                        "[NotifyClientDisConnected] failed for listener {}",
                        clientConnectionEventListener.getName(),
                        throwable);
            }
        }
    ...
    public void registerClientConnectionEventListener(ClientConnectionEventListener listener) {
        Loggers.REMOTE.info(
                "[ClientConnectionEventListenerRegistry] registry listener - "
