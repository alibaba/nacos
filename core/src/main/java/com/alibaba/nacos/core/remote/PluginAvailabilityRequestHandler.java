...
/**
 * Plugin availability request handler.
 * Handles plugin availability queries from other cluster nodes.
 *
 * @author WangzJi
 * @since 3.2.0
 */
...
public class PluginAvailabilityRequestHandler
    ...
    public PluginAvailabilityResponse handle(PluginAvailabilityRequest request, RequestMeta meta)
        ...
        response.setPluginId(request.getPluginId());

        // Check if plugin exists locally
        boolean available = pluginManager.isPluginAvailable(request.getPluginId());
