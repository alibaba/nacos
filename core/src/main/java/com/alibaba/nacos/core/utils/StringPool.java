...
public class StringPool {
    ...
    static {
        GROUP_KEY_CACHE = CacheBuilder.builder().maximumSize(5000000) .expireNanos(180, TimeUnit.SECONDS).buildAutoExpireCache(
                CacheBuilder.builder().buidLruCache(
                    CacheBuilder.builder().buildCache()
                )
        );
    }
