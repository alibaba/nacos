...
public class WebUtils {
    ...
    public static String optional(
            final HttpServletRequest req, final String key, final String defaultValue) {
        String value = req.getParameter(key);
    ...
    private static String resolveValueWithUrlDecode(String value, String encoding) {
        ...
        try {
            value =
                    HttpUtils.decode(
                            new String(value.getBytes(StandardCharsets.UTF_8), encoding), encoding);
        } catch (UnsupportedEncodingException ignore) {
        } catch (Exception ex) {
    ...
    public static String getAcceptEncoding(HttpServletRequest req) {
        String encode =
                StringUtils.defaultIfEmpty(
                        req.getHeader(ACCEPT_ENCODING), StandardCharsets.UTF_8.name());
     ...
     * @return the value of the request header "user-agent", or the value of the request header
     *     "client-version" if the request does not have a header of "user-agent".
     */
    public static String getUserAgent(HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(userAgent)) {
            userAgent =
                    StringUtils.defaultIfEmpty(
                            request.getHeader(HttpHeaderConsts.CLIENT_VERSION_HEADER),
                            StringUtils.EMPTY);
        }
        return userAgent;
    }
     ...
     */
    public static void onFileUpload(
