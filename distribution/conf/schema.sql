...
type varchar(64) DEFAULT NULL,
c_schema LONG VARCHAR DEFAULT NULL,
constraint configinfodraft_id_key PRIMARY KEY (id),
constraint uk_configinfodraft_datagrouptenant UNIQUE (data_id,group_id,tenant_id));

CREATE INDEX configinfodraft_dataid_key_idx ON config_info_draft(data_id);
CREATE INDEX configinfodraft_groupid_key_idx ON config_info_draft(group_id);
CREATE INDEX configinfodraft_dataid_group_key_idx ON config_info_draft(data_id, group_id);
...
tenant_id varchar(128) DEFAULT '',
nid bigint NOT NULL generated by default as identity,
constraint config_tags_draft_id_key PRIMARY KEY (nid),
constraint uk_configtagrelationdraft_configidtag UNIQUE (id, tag_name, tag_type));

CREATE INDEX config_tags_draft_tenant_id_idx ON config_tags_relation_draft(tenant_id);
