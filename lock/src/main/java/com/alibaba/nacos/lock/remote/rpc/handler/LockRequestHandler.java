...
public class LockRequestHandler extends RequestHandler<LockOperationRequest, LockOperationResponse> {
    ...
    public LockOperationResponse handle(LockOperationRequest request, RequestMeta meta) throws NacosException {
        LOGGER.debug("request: {}, instance: {}", request.getLockOperationEnum(), request.getLockInstance());
        try {
            ...
            if (request.getLockOperationEnum() == LockOperationEnum.ACQUIRE) {
                if (lockInstance.getExpiredTime() == 0) {
                    return LockOperationResponse.fail("Lock expiredTime must be non-zero for ACQUIRE");
                }
                ...
                if (lockInstance.getExpiredTime() == 0) {
                    return LockOperationResponse.fail("Lock expiredTime must be non-zero for RENEW");
                }
