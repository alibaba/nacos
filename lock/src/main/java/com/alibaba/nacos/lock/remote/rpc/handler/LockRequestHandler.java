...
public class LockRequestHandler extends RequestHandler<LockOperationRequest, LockOperationResponse> {
    ...
    public LockOperationResponse handle(LockOperationRequest request, RequestMeta meta) throws NacosException {
        Boolean lock = null;
        if (request.getLockOperation() == LockOperation.ACQUIRE) {
            lock = lockOperationService.lock(request.getLockInfo(), meta.getConnectionId());
        } else if (request.getLockOperation() == LockOperation.RELEASE) {
            lock = lockOperationService.unLock(request.getLockInfo());
        } else {
