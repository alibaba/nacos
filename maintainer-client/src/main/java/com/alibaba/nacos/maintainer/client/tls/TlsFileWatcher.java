...
package com.alibaba.nacos.maintainer.client.tls;

import com.alibaba.nacos.maintainer.client.constants.PropertyKeyConstants;
import com.alibaba.nacos.maintainer.client.executor.ExecutorFactory;
import com.alibaba.nacos.maintainer.client.executor.NameThreadFactory;
import com.alibaba.nacos.maintainer.client.utils.ClassUtils;
        ...
        for (String filePath : filePaths) {
            if (filePath != null && new File(filePath).exists()) {
                ...
                try {
                    ...
                    fileMd5Map.put(filePath, MD5Utils.md5Hex(IoUtils.toString(in, PropertyKeyConstants.ENCODE), PropertyKeyConstants.ENCODE));
                } finally {
        ...
        if (started.compareAndSet(false, true)) {
            service.scheduleAtFixedRate(() -> {
                for (Map.Entry<String, FileChangeListener> item : watchFilesMap.entrySet()) {
                    ...
                    try {
                        in = new FileInputStream(filePath);
                        newHash = MD5Utils.md5Hex(IoUtils.toString(in, PropertyKeyConstants.ENCODE), PropertyKeyConstants.ENCODE);
