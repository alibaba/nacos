...
public class HttpHealthCheckProcessor implements HealthCheckProcessorV2 {
        ...
        try {
            ...
            if (!instance.tryStartCheck()) {
                ...
                healthCheckCommon.reEvaluateCheckRT(
                        task.getCheckRtNormalized() * 2, task, switchDomain.getHttpHealthParams());
                return;
            }

            Http healthChecker = (Http) metadata.getHealthChecker();
            int ckPort =
                    metadata.isUseInstancePortForCheck()
                            ? instance.getPort()
    ...
    private class HttpHealthCheckCallback implements Callback<String> {
        ...
        public HttpHealthCheckCallback(
                HealthCheckInstancePublishInfo instance, HealthCheckTaskV2 task, Service service) {
        ...
        public void onError(Throwable throwable) {
            ...
            if (throwable instanceof ConnectException) {
                healthCheckCommon.checkFailNow(
                        task, service, "http:unable2connect:" + throwable.getMessage());
            } else {
