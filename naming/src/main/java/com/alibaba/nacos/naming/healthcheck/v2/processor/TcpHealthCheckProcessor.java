...
public class TcpHealthCheckProcessor implements HealthCheckProcessorV2, Runnable {
    ...
    public void process(HealthCheckTaskV2 task, Service service, ClusterMetadata metadata) {
        ...
        if (!instance.tryStartCheck()) {
            ...
            healthCheckCommon.reEvaluateCheckRT(
                    task.getCheckRtNormalized() * 2, task, switchDomain.getTcpHealthParams());
            return;
        }
        ...
        public void run() {
            ...
            try {
                ...
                beat.finishCheck(
                        false,
                        true,
                        switchDomain.getTcpHealthParams().getMax(),
    ...
    private class Beat {
        ...
        public String toString() {
            return service.getGroupedServiceName()
                    + ":"
                    + instance.getCluster()
                    + ":"
                    + instance.getIp()
    ...
    private static class TimeOutTask implements Runnable {
        ...
        public void run() {
            if (key != null && key.isValid()) {
                ...
                try {
                    beat.finishCheck(
                            false, false, beat.getTask().getCheckRtNormalized() * 2, "tcp:timeout");
                    key.cancel();
                    key.channel().close();
                } catch (Exception ignore) {
                }
    ...
    private class TaskProcessor implements Callable<Void> {
        ...
        public Void call() {
            ...
            try {
                ...
                int port =
                        cluster.isUseInstancePortForCheck()
                                ? instance.getPort()
                ...
                GlobalExecutor.scheduleTcpSuperSenseTask(
                        new TimeOutTask(key), CONNECT_TIMEOUT_MS, TimeUnit.MILLISECONDS);
            } catch (Exception e) {
                beat.finishCheck(
                        false,
                        false,
