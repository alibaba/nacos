...
public class HttpClientManager {
    ...
    static {
        ...
        PROCESSOR_NACOS_ASYNC_REST_TEMPLATE =
                HttpClientBeanHolder.getNacosAsyncRestTemplate(PROCESSOR_ASYNC_HTTP_CLIENT_FACTORY);
    ...
    /**
     * To be compatible with the old version of http client request, this NacosAsyncRestTemplate is
     * only used for HttpHealthCheckProcessor.
     *
    ...
    private static void shutdown() {
        ...
        try {
            HttpClientBeanHolder.shutdownNacostSyncRest(
                    SYNC_HTTP_CLIENT_FACTORY.getClass().getName());
            HttpClientBeanHolder.shutdownNacostSyncRest(
                    APACHE_SYNC_HTTP_CLIENT_FACTORY.getClass().getName());
            HttpClientBeanHolder.shutdownNacosAsyncRest(
                    ASYNC_HTTP_CLIENT_FACTORY.getClass().getName());
            HttpClientBeanHolder.shutdownNacosAsyncRest(
                    PROCESSOR_ASYNC_HTTP_CLIENT_FACTORY.getClass().getName());
        } catch (Exception ex) {
            SRV_LOG.error(
    ...
    private static class ApacheSyncHttpClientFactory extends AbstractApacheHttpClientFactory {
        ...
        protected HttpClientConfig buildHttpClientConfig() {
            return HttpClientConfig.builder()
                    ...
                    .setMaxConnPerRoute(EnvUtil.getAvailableProcessors())
                    .setMaxRedirects(0)
                    .build();
        }
