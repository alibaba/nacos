...
/**
 * Aggregation naming subscriber service. Aggregate all implementation of {@link
 * NamingSubscriberService} and subscribers from other nodes.
 *
 * @author xiweng.yy
 */
...
public class NamingSubscriberServiceAggregationImpl implements NamingSubscriberService {
    ...
    public Collection<Subscriber> getSubscribers(String namespaceId, String serviceName) {
        Collection<Subscriber> result =
                new LinkedList<>(subscriberServiceLocal.getSubscribers(namespaceId, serviceName));
    ...
    public Collection<Subscriber> getSubscribers(Service service) {
        ...
        if (memberManager.getServerList().size() > 1) {
            getSubscribersFromRemotes(
                    service.getNamespace(), service.getGroupedServiceName(), result);
        }
        return result;
    }
    ...
    private void getSubscribersFromRemotes(
            String namespaceId, String serviceName, Collection<Subscriber> result) {
