...
public class PushExecutorUdpImpl implements PushExecutor {
    ...
    public void doPush(String clientId, Subscriber subscriber, PushDataWrapper data) {
        pushService.pushDataWithoutCallback(
                subscriber,
    ...
    public void doPushWithCallback(
        ...
        pushService.pushDataWithCallback(
                subscriber, handleClusterData(actualServiceInfo, subscriber), callBack);
    }

    /**
     * The reason to replace the name is upd push is used in 1.x client. And 1.x client do not
     * identify the group attribute but only identify name attribute. So for supporting 1.x client,
     * replace it with a new {@link ServiceInfo}.
     *
     * <p>Why not setName directly? Because the input {@link ServiceInfo} may be reused by 2.x push
     * execute. And if set name directly will has some effect for 2.x client.
     *
     * @param originalData original service info
     * @return new service info for 1.x
     */
    private ServiceInfo replaceServiceInfoName(
            PushDataWrapper originalData, Subscriber subscriber) {
        ServiceInfo serviceInfo =
                ServiceUtil.selectInstancesWithHealthyProtection(
                        originalData.getOriginalData(),
                        originalData.getServiceMetadata(),
                        false,
                        true,
                        subscriber);
        ServiceInfo result = new ServiceInfo();
        result.setName(
                NamingUtils.getGroupedName(serviceInfo.getName(), serviceInfo.getGroupName()));
    ...
    private ServiceInfo handleClusterData(ServiceInfo data, Subscriber subscriber) {
        return StringUtils.isBlank(subscriber.getCluster())
                ? data
