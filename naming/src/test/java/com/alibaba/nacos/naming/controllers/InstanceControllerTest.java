...
public class InstanceControllerTest extends BaseTest {
    ...
    @InjectMocks private InstanceController instanceController;
    ...
    public void testRegister() throws Exception {
        ...
        verify(instanceServiceV2)
                .registerInstance(
                        ...
                        eq(TEST_GROUP_NAME + "@@" + TEST_SERVICE_NAME),
                        any(Instance.class));
    ...
    public void testBatchUpdateInstanceMetadata() throws Exception {
        ...
        when(instanceServiceV2.batchUpdateMetadata(
                        eq(Constants.DEFAULT_NAMESPACE_ID), any(), anyMap()))
        ...
        assertEquals(
                "1.1.1.1:3306:unknown:DEFAULT:ephemeral", actual.get("updated").get(0).textValue());
    }

    @Test
    public void testBatchDeleteInstanceMetadata() throws Exception {
        mockRequestParameter("metadata", "{}");
        when(instanceServiceV2.batchDeleteMetadata(
                        eq(Constants.DEFAULT_NAMESPACE_ID), any(), anyMap()))
                .thenReturn(Collections.singletonList("1.1.1.1:3306:unknown:DEFAULT:ephemeral"));
    ...
    public void testDetail() throws Exception {
        ...
        when(instanceServiceV2.getInstance(
                        ...
                        TEST_GROUP_NAME + "@@" + TEST_SERVICE_NAME,
    ...
    public void testBeat() throws Exception {
        when(instanceServiceV2.handleBeat(
                        eq(Constants.DEFAULT_NAMESPACE_ID),
                        eq(TEST_GROUP_NAME + "@@" + TEST_SERVICE_NAME),
                        eq("1.1.1.1"),
                        eq(3306),
                        eq(UtilsAndCommons.DEFAULT_CLUSTER_NAME),
                        any(),
                        any()))
                .thenReturn(200);
        when(instanceServiceV2.getHeartBeatInterval(
                        eq(Constants.DEFAULT_NAMESPACE_ID),
                        eq(TEST_GROUP_NAME + "@@" + TEST_SERVICE_NAME),
                        eq("1.1.1.1"),
                        eq(3306),
