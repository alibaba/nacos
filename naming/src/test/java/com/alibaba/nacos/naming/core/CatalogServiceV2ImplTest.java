...
public class CatalogServiceV2ImplTest {
    ...
    public void testGetServiceDetail() throws NacosException {
        ...
        Assert.assertEquals(
                "C", objectNode.get(FieldsConstants.SERVICE).get(FieldsConstants.NAME).asText());
        Assert.assertEquals(
                "B",
                objectNode.get(FieldsConstants.SERVICE).get(FieldsConstants.GROUP_NAME).asText());
        Assert.assertEquals(
                "none",
                objectNode
                        .get(FieldsConstants.SERVICE)
                        .get(FieldsConstants.SELECTOR)
                        .get("type")
                        .asText());
        Assert.assertEquals(
                0, objectNode.get(FieldsConstants.SERVICE).get(FieldsConstants.METADATA).size());
        Assert.assertEquals(
                0.75,
                objectNode
                        .get(FieldsConstants.SERVICE)
                        .get(FieldsConstants.PROTECT_THRESHOLD)
                        .asDouble(),
                0.1);
    }
    ...
    public void testPageListService() throws NacosException {
        ...
        ObjectNode obj =
                (ObjectNode)
                        catalogServiceV2Impl.pageListService("A", "B", "C", 1, 10, null, false);
    ...
    public void testPageListServiceForPage() throws NacosException {
        ...
        ObjectNode obj =
                (ObjectNode)
                        catalogServiceV2Impl.pageListService(
                                "CatalogService", "", "", 2, 1, null, false);
