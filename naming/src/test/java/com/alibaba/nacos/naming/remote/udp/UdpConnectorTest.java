...
public class UdpConnectorTest {
    ...
    @InjectMocks private UdpConnector udpConnector;

    @Mock private ConcurrentMap<String, AckEntry> ackMap;

    @Mock private ConcurrentMap<String, PushCallBack> callbackMap;

    @Mock private DatagramSocket udpSocket;
    ...
    public void setUp() throws IOException, InterruptedException {
        ...
        DatagramSocket oldSocket =
                (DatagramSocket) ReflectionTestUtils.getField(udpConnector, "udpSocket");
    ...
    public void testSendDataWithCallback() throws IOException, InterruptedException {
        when(udpSocket.isClosed()).thenReturn(false);
        AckEntry ackEntry = new AckEntry("A", new DatagramPacket(new byte[2], 2));
        udpConnector.sendDataWithCallback(
                ackEntry,
                new PushCallBack() {
                    @Override
