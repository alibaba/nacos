...
public interface BaseDatabaseOperate extends DatabaseOperate {
    ...
    default <R> R queryOne(JdbcTemplate jdbcTemplate, String sql, Class<R> cls) {
        try {
            ...
            LOGGER.error(
                    "[db-error] can't get connection : {}", ExceptionUtil.getAllExceptionMsg(e));
            throw e;
        } catch (DataAccessException e) {
            LOGGER.error(
                    "[db-error] DataAccessException : {}", ExceptionUtil.getAllExceptionMsg(e));
            throw e;
        }
    }
    ...
    /**
     * query one result by sql and args then convert result to target type through {@link
     * RowMapper}.
     *
     * @param jdbcTemplate {@link JdbcTemplate}
     * @param sql sql
     ...
     * @return R
     */
    default <R> R queryOne(
            JdbcTemplate jdbcTemplate, String sql, Object[] args, RowMapper<R> mapper) {
        try {
    ...
    /**
     * query many result by sql and args then convert result to target type through {@link
     * RowMapper}.
     *
     * @param jdbcTemplate {@link JdbcTemplate}
     * @param sql sql
     ...
     */
    default <R> List<R> queryMany(
            JdbcTemplate jdbcTemplate, String sql, Object[] args, Class<R> rClass) {
        try {
    ...
    /**
     * query many result by sql and args then convert result to List&lt;Map&lt;String,
     * Object&gt;&gt;.
     *
     * @param jdbcTemplate {@link JdbcTemplate}
     * @param sql sql
     ...
     */
    default List<Map<String, Object>> queryMany(
    ...
    default Boolean update(
        ...
        try {
                                ...
                                try {
                                    contexts.forEach(
                                            pair -> {
                                                ...
                                                args[0] = pair.getArgs();
                                                boolean rollBackOnUpdateFail =
                                                        pair.isRollBackOnUpdateFail();
                                                LoggerUtils.printIfDebugEnabled(
                                                        LOGGER, "current sql : {}", errSql[0]);
                                                LoggerUtils.printIfDebugEnabled(
                                                        LOGGER, "current args : {}", args[0]);
                                                int row =
                                                        jdbcTemplate.update(
            ...
            LoggerUtils.printIfDebugEnabled(
                    LOGGER, "Roll back transaction for {} ", e.getMessage());
            if (consumer != null) {
                consumer.accept(Boolean.FALSE, e);
            }
        }
    ...
    default Boolean doDataImport(JdbcTemplate template, List<ModifyRequest> requests) {
        final String[] sql =
                requests.stream()
                        .map(ModifyRequest::getSql)
                        .map(DerbyUtils::insertStatementCorrection)
                        .toArray(String[]::new);
