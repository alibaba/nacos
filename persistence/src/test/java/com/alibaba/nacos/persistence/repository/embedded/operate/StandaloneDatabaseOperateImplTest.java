...
public class StandaloneDatabaseOperateImplTest {
    ...
    @Spy @InjectMocks private StandaloneDatabaseOperateImpl operate;

    @Mock private RowMapper<MockConfigInfo> rowMapper;

    @Mock private JdbcTemplate jdbcTemplate;

    @Mock private JdbcTemplate tempJdbcTemplate;

    @Mock private BiConsumer<Boolean, Throwable> biConsumer;
    ...
    */
    ...
    public void testQueryOne5() {
        ...
        when(tempJdbcTemplate.queryForObject(sql, args, MockConfigInfo.class))
                .thenReturn(configInfo);
        Assert.assertEquals(
                operate.queryOne(tempJdbcTemplate, sql, args, MockConfigInfo.class), configInfo);
    }
    ...
    public void testDataImport() throws ExecutionException, InterruptedException {
        RestResult<String> errorResult =
                RestResult.<String>builder().withCode(500).withMsg("null").withData(null).build();
    ...
    public void testUpdate4() {
        ...
        when(transactionTemplate.execute(any(TransactionCallback.class))).thenReturn(true);
        Assert.assertTrue(
                operate.update(transactionTemplate, jdbcTemplate, modifyRequests, biConsumer));
    }
