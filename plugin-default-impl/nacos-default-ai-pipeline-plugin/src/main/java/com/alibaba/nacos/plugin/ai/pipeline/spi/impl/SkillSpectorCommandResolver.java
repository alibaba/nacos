...
/**
 * Resolves the configured skill-spector command against the server environment.
 *
 * @author Nacos
 */
final class SkillSpectorCommandResolver {
    
    private static final Logger LOGGER = LoggerFactory.getLogger(SkillSpectorCommandResolver.class);
    
    private static final String USER_AI_PIPELINE_BIN = "ai-infra/ai-pipeline/bin";
    
    private SkillSpectorCommandResolver() {
    }
    ...
    static String resolve(String configuredCommand, String pathEnv, String userHome) {
        String command = StringUtils.isBlank(configuredCommand)
            ? SkillSpectorPluginConfig.DEFAULT_COMMAND : configuredCommand.trim();
        String resolved = resolveCandidate(command, pathEnv, userHome);
        if (StringUtils.isNotBlank(resolved)
            || SkillSpectorPluginConfig.DEFAULT_COMMAND.equals(command)) {
            return resolved;
        }
        return resolveCandidate(SkillSpectorPluginConfig.DEFAULT_COMMAND, pathEnv, userHome);
    }
