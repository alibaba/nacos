...
public class LdapAuthenticationProviderTest {
    ...
    public void setUp() throws NoSuchMethodException {
        ...
        when(ldapTemplate.authenticate(
                        "", "(" + filterPrefix + "=" + adminUserName + ")", defaultPassWord))
                .thenAnswer(
                        new Answer<Boolean>() {
                            @Override
        ...
        ldapLogin =
                LdapAuthenticationProvider.class.getDeclaredMethod(
                        "ldapLogin", String.class, String.class);
    ...
    public void testldapLogin() {
        try {
            Boolean result =
                    (Boolean)
                            ldapLogin.invoke(
                                    ldapAuthenticationProvider, adminUserName, defaultPassWord);
            Assert.assertTrue(result);
        } catch (IllegalAccessException e) {
            Assert.fail();
        } catch (InvocationTargetException e) {
    ...
    public void testDefaultCaseSensitive() {
        ...
        when(ldapTemplate.authenticate(
                        "", "(" + filterPrefix + "=" + userName + ")", defaultPassWord))
                .thenAnswer(
                        new Answer<Boolean>() {
                            @Override
                            public Boolean answer(InvocationOnMock invocation) throws Throwable {
    ...
    public void testCloseCaseSensitive() {
        when(ldapTemplate.authenticate(
                        "", "(" + filterPrefix + "=" + normalUserName + ")", defaultPassWord))
                .thenAnswer(
                        new Answer<Boolean>() {
                            @Override
                            public Boolean answer(InvocationOnMock invocation) throws Throwable {
