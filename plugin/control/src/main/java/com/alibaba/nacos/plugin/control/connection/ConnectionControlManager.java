...
public abstract class ConnectionControlManager {
    ...
    public ConnectionControlManager() {
        ...
        Loggers.CONTROL.info(
                "Load connection metrics collector,size={},{}",
    ...
    private void initExecuteService() {
        executorService =
                ExecutorFactory.newSingleScheduledExecutorService(
                        r -> {
                            Thread thread =
                                    new Thread(r, "nacos.plugin.control.connection.reporter");
    ...
    private void startConnectionMetricsReport() {
        executorService.scheduleWithFixedDelay(
                new ConnectionMetricsReporter(), 0, 3000, TimeUnit.MILLISECONDS);
    }
    ...
    private void initConnectionRule() {
        ...
        if (StringUtils.isNotBlank(localRuleContent)) {
            Loggers.CONTROL.info(
                    "Found local disk connection rule content on start up,value  ={}",
                    localRuleContent);
        ...
        if (StringUtils.isNotBlank(localRuleContent)) {
            connectionControlRule =
                    RuleParserProxy.getInstance().parseConnectionRule(localRuleContent);
    ...
    class ConnectionMetricsReporter implements Runnable {
        ...
        @Override
        public void run() {
            Map<String, Integer> metricsTotalCount =
                    metricsCollectorList.stream()
                            .collect(
                                    Collectors.toMap(
                                            ConnectionMetricsCollector::getName,
            ...
            Loggers.CONNECTION.info(
                    String.format(
                            "ConnectionMetrics, totalCount = %s, detail = %s",
                            totalCount, metricsTotalCount.toString()));
        }
    }
}
