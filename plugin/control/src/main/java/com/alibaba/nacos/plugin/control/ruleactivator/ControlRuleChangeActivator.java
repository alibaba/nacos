...
public class ControlRuleChangeActivator {
    ...
    class TpsRuleChangeSubscriber extends Subscriber<TpsControlRuleChangeEvent> {
        ...
        public void onEvent(TpsControlRuleChangeEvent event) {
            ...
            LOGGER.info(
                    "Tps control rule change event receive,pointName={}, external={} ",
                    pointName,
                    event.isExternal());
            ...
            try {
                ...
                if (event.isExternal()) {
                    if (ruleStorageProxy.getExternalStorage() != null) {
                        ...
                        ruleStorageProxy
                                .getLocalDiskStorage()
                                .saveTpsRule(pointName, persistTpsRule);
                    } else {
                ...
                TpsControlRule tpsControlRule =
                        StringUtils.isBlank(tpsRuleContent)
                                ? new TpsControlRule()
                                : ControlManagerCenter.getInstance()
                                        .getRuleParser()
                                        .parseTpsRule(tpsRuleContent);

                ControlManagerCenter.getInstance()
                        .getTpsControlManager()
                        .applyTpsRule(pointName, tpsControlRule);
    ...
    class ConnectionRuleChangeSubscriber extends Subscriber<ConnectionLimitRuleChangeEvent> {
        ...
        public void onEvent(ConnectionLimitRuleChangeEvent event) {
            LOGGER.info(
                    "connection limit rule change event receive ,external:{}", event.isExternal());
            ...
            try {
                ...
                if (event.isExternal()) {
                    if (ruleStorageProxy.getExternalStorage() != null) {
                        ...
                        Loggers.CONTROL.info(
                                "No external rule storage found,will load local disk instead");
                    }
                }
                ...
                ConnectionControlRule connectionControlRule =
                        StringUtils.isBlank(limitRule)
                                ? new ConnectionControlRule()
                                : ControlManagerCenter.getInstance()
                                        .getRuleParser()
                                        .parseConnectionRule(limitRule);
                ...
                LOGGER.error(
                        "Fail to parse connection limit rule ,persit:{}", event.isExternal(), e);
            }
        }
