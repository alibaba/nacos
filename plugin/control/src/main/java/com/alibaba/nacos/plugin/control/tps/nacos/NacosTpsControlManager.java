...
public class NacosTpsControlManager extends TpsControlManager {
    ...
    protected void startTpsReport() {
        executorService.scheduleWithFixedDelay(
                new NacosTpsControlManager.TpsMetricsReporter(), 0, 900, TimeUnit.MILLISECONDS);
    }
    ...
    public synchronized void registerTpsPoint(String pointName) {
        if (!points.containsKey(pointName)) {
            points.put(
                    pointName,
                    TpsBarrierCreatorProxy.getTpsBarrierCreator().createTpsBarrier(pointName));
    ...
    public TpsCheckResponse check(TpsCheckRequest tpsRequest) {
        ...
        if (points.containsKey(tpsRequest.getPointName())) {
            try {
                ...
                Loggers.TPS.warn(
                        "[{}]apply tps error,error={}", tpsRequest.getPointName(), throwable);
            }
        }
                ...
                for (Map.Entry<String, TpsBarrier> entry : entries) {
                    ...
                    if (metrics != null) {
                        ...
                        stringBuilder
                                ...
                                .append("point")
                                .append("|")
                                .append(metrics.getPeriod())
                                .append("|")
                                .append(formatString)
                                .append("|")
                                .append(metrics.getCounter().getPassCount())
                                .append("|")
                                .append(metrics.getCounter().getDeniedCount())
                                .append("|")
                                .append("\n");
                    }
                }
