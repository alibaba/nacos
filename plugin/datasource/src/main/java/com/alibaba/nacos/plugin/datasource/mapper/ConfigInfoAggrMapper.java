...
public interface ConfigInfoAggrMapper extends Mapper {
    ...
    /**
     * To delete aggregated data in bulk, you need to specify a size of datum list. The default sql:
    ...
    default MapperResult batchRemoveAggr(MapperContext context) {
        final List<String> datumList =
                (List<String>) context.getWhereParameter(FieldConstant.DATUM_ID);
    ...
    /**
     * Get count of aggregation config info. The default sql: SELECT count(*) FROM config_info_aggr
    ...
    /**
     * Find all data before aggregation under a dataId. It is guaranteed not to return NULL. The
     * default sql: SELECT data_id,group_id,tenant_id,datum_id,app_name,content FROM
    ...
    /**
     * Query aggregation config info. The default sql: SELECT
     * data_id,group_id,tenant_id,datum_id,app_name,content FROM config_info_aggr WHERE data_id=?
     * AND group_id=? AND tenant_id=? ORDER BY datum_id LIMIT startRow,pageSize
     *
    ...
    /**
     * Find all aggregated data sets. The default sql: SELECT DISTINCT data_id, group_id, tenant_id
