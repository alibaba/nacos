...
public interface ConfigInfoMapper extends Mapper {
    ...
    /**
     * Find all dataId and group. The default sql: SELECT DISTINCT data_id, group_id FROM
     * config_info
     *
     * @param context sql paramMap
    ...
    default MapperResult findAllDataIdAndGroup(MapperContext context) {
        return new MapperResult(
                "SELECT DISTINCT data_id, group_id FROM config_info", Collections.emptyList());
    }

    /**
     * Query the count of config_info by tenantId and appName. The default sql: SELECT count(*) FROM
     * config_info WHERE tenant_id LIKE ? AND app_name=?
     *
     * @param context sql paramMap
    ...
    /**
     * Query configuration information based on group. <br>
     * The default sql: SELECT id,data_id,group_id,tenant_id,app_name,content FROM config_info WHERE
    ...
    /**
     * Returns the number of configuration items. The default sql: SELECT count(*) FROM config_info
    ...
    /**
     * Get tenant id list by page. The default sql: SELECT tenant_id FROM config_info WHERE
     * tenant_id != '' GROUP BY tenant_id LIMIT startRow, pageSize
     *
     * @param context The context of startRow, pageSize
    ...
    /**
     * Query all configuration information by page. The default sql: SELECT
     * data_id,group_id,app_name FROM ( SELECT id FROM config_info WHERE tenant_id LIKE ? ORDER BY
     * id LIMIT startRow, pageSize ) g, config_info t WHERE g.id = t.id "
     *
     * @param context The context of startRow, pageSize
    ...
    /**
     * Query all configuration information by page. The default sql: SELECT
     * t.id,data_id,group_id,content,md5 FROM ( SELECT id FROM config_info ORDER BY id LIMIT ?,?) g,
     ...
     * id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,type,encrypted_data_key FROM
     * config_info WHERE id > ? ORDER BY id ASC LIMIT startRow,pageSize
     *
     * @param context The context of startRow, pageSize
    ...
    /**
     * Query change config. <br>
     * The default sql: SELECT data_id, group_id, tenant_id, app_name, content,
     ...
     * configuration. The default sql: SELECT
     * id,data_id,group_id,tenant_id,app_name,content,type,md5,gmt_modified FROM config_info WHERE
     * ...
     *
     * @param context The map of params, the key is the parameter name(dataId, groupId, tenantId,
     *     appName, startTime, endTime, content, startTime, endTime), the value is the key's value.
     ...
     * t.id,data_id,group_id,tenant_id,app_name,md5,type,gmt_modified,encrypted_data_key FROM (
    ...
    /**
     * query all configuration information according to group, appName, tenant (for export). The
     * default sql: SELECT
     ...
     *
     * @param context The map of params, the key is the parameter name(dataId, group, appName), the
     ...
     * ...
     *
     * @param context The map of params, the key is the parameter name(dataId, groupId, tenant_id,
     *     content), the value is the arbitrary object.
     * @return The sql of getting the count of config information.
     */
    ...
    /**
     * Get the config information. The default sql: SELECT id,data_id,group_id,tenant_id,content
     * FROM config_info WHERE ...
     *
     * @param context The map of params, the key is the parameter name(dataId, groupId, tenant_id,
     *     content), the value is the key's value.
     ...
     * id,data_id,group_id,tenant_id,app_name,content,type,encrypted_data_key FROM config_info ...
     *
     * @param context The mpa of dataId, groupId and appName.
     ...
     * id,data_id,group_id,content FROM config_info WHERE group_id=? AND tenant_id=?
     *
     * @param context The context of startRow, pageSize
    ...
    /**
     * Query all configuration information by page. <br>
     * The default sql: <br>
     * SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5 " + " FROM ( SELECT id FROM
     * config_info WHERE tenant_id LIKE ? ORDER BY id LIMIT ?,? )" + " g, config_info t WHERE g.id =
     * t.id
     *
     * @param context The context of startRow, pageSize
     ...
     * encapsulation. The default sql: UPDATE config_info SET content=?, md5 = ?,
     * src_ip=?,src_user=?,gmt_modified=?, app_name=?,c_desc=?,c_use=?, effect=?,type=?,c_schema=?
