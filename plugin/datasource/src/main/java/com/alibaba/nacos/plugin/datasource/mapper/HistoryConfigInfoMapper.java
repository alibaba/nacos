...
public interface HistoryConfigInfoMapper extends Mapper {
    ...
    /**
     * Delete data before startTime. The default sql: DELETE FROM his_config_info WHERE gmt_modified
     * < ? LIMIT ?
     *
    ...
    /**
     * Get the number of configurations before the specified time. The default sql: SELECT count(*)
     * FROM his_config_info WHERE gmt_modified < ?
     *
     * @param context sql paramMap
    ...
    /**
     * Query deleted config. The default sql: SELECT DISTINCT data_id, group_id, tenant_id FROM
     * his_config_info WHERE op_type = 'D' AND gmt_modified >=? AND gmt_modified <= ?
     *
     * @param context sql paramMap
     ...
     * nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified FROM
     * his_config_info WHERE data_id = ? AND group_id = ? AND tenant_id = ? ORDER BY nid DESC
     *
     * @param context sql paramMap
     ...
     * nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified FROM
     * his_config_info WHERE data_id = ? AND group_id = ? AND tenant_id = ? ORDER BY nid DESC limit
     * ?,?
     *
