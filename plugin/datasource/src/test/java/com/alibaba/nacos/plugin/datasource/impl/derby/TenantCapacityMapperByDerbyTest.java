...
public class TenantCapacityMapperByDerbyTest {
    ...
    public void testIncrementUsageWithDefaultQuotaLimit() {
        MapperResult mapperResult =
                tenantCapacityMapperByDerby.incrementUsageWithDefaultQuotaLimit(context);
        Assert.assertEquals(
                mapperResult.getSql(),
                "UPDATE tenant_capacity SET usage = usage + 1, gmt_modified = ? WHERE tenant_id = ? AND usage <"
                        + " ? AND quota = 0");
        Assert.assertArrayEquals(
                mapperResult.getParamList().toArray(), new Object[] {modified, tenantId, usage});
    }

    @Test
    public void testIncrementUsageWithQuotaLimit() {
        MapperResult mapperResult =
                tenantCapacityMapperByDerby.incrementUsageWithQuotaLimit(context);
        Assert.assertEquals(
                mapperResult.getSql(),
                "UPDATE tenant_capacity SET usage = usage + 1, gmt_modified = ? WHERE tenant_id = ? AND usage < "
                        + "quota AND quota != 0");
        Assert.assertArrayEquals(
                mapperResult.getParamList().toArray(), new Object[] {modified, tenantId});
    }
    ...
    public void testCorrectUsage() {
        ...
        Assert.assertArrayEquals(
                mapperResult.getParamList().toArray(), new Object[] {tenantId, modified, tenantId});
    }
    ...
    public void testGetCapacityList4CorrectUsage() {
        ...
        MapperResult mapperResult =
                tenantCapacityMapperByDerby.getCapacityList4CorrectUsage(context);
        Assert.assertEquals(
                mapperResult.getSql(),
