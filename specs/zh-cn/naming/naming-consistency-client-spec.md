...
临时服务拥有偏 AP 的运行时实例状态。实例注册在临时 client 下，通过 Distro client data 路径在集群
中同步，并在心跳或连接生命周期表明 client 消失时移除。
...
persistent client operation service apply。持久实例 snapshot 用于状态恢复。
...
- 临时服务的 Distro data ownership、verify flow 和 anti-entropy 行为应在共享 AP 一致性规范中定义。
- Naming metadata 和持久服务使用的 CP protocol group 应在共享持久化/一致性规范中定义。
