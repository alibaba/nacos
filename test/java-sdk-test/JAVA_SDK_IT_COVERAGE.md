...
| `AgentDiscoveryService` / `AiService` | `AgentDiscoveryServiceJavaSdkITCase` | Covered | Verifies default/custom namespace isolation and binding, caller immutability, default/individual/combined/empty/paged Search, latest/exact/label and filtered Discover, complete Endpoint publication replacement, idempotence, canonical natural-key partial and final deregistration, protocol isolation, two-publisher aggregation, pre-registration, subscribe-before-create and subscribe-existing polling, complete-fingerprint change/de-duplication, unsubscribe suppression, Version evolution under Endpoint-first and definition-first ordering, catalog/latest/label/exact subscription consistency, offline/online latest recalculation, inclusive Version-range replacement, active HTTP shutdown cleanup, HTTP/gRPC parity, local validation boundaries, not-found/error mapping, and both default and Jackson 3 JSON adapters. Compatibility coverage releases through the legacy A2A SDK, verifies the canonical Console and RAD projections, registers an old exact-Version Endpoint and verifies both the legacy SERVICE result and intentional empty new Runtime snapshot, rejects duplicate overwrite, then publishes a complete A2A Version through the canonical Maintainer SDK and verifies legacy query plus latest polling subscription convergence. An opt-in directed IT stops and restarts a real standalone server while the same SDK process verifies connection failure, reconnect, gRPC publication redo, HTTP `50404` publication replay, polling recovery, independent legacy Version 1 and pre-registered Version 2 Endpoint redo, and a later Version/Endpoint transition. | Server Watch/Push and public local-selection helpers are explicitly deferred by the scoped first-version contract. Ability changes, transport-result ambiguity, non-50404 heartbeat failures, listener failure, rollback, and redo races remain deterministic client unit-test scenarios rather than unstable shared-server fault injection. |
