...
| `AgentDiscoveryService` / `AiService` | `AgentDiscoveryServiceJavaSdkITCase` | Covered | Verifies default/custom namespace isolation and binding; immutable caller models; literal, typed, paged, and convergent Search; omitted/latest/exact/label and fully filtered Discover; complete Endpoint replacement, idempotence, partial/final deregistration, protocol isolation, two-publisher aggregation, pre-registration, Version ranges, and rollout-safe multi-Version pools. Subscription coverage includes subscribe-before-create `UNAVAILABLE` to complete `SNAPSHOT`, negotiated gRPC fingerprint Hint followed by authoritative Discover, HTTP/ability-missing polling compatibility, canonical intent sharing, multiple and throwing listeners, partial/final unsubscribe, shutdown suppression, complete-fingerprint de-duplication, local subscription capacity, authoritative server gRPC Watch soft-watermark rejection/cleanup/reuse, and publication capacity/redo cleanup. Transport coverage includes HTTP/gRPC parity, stable cross-transport canonical fingerprints, AUTO with available gRPC, AUTO immediate HTTP when gRPC remains STARTING, explicit HTTP independence, explicit GRPC no operation fallback, validation/error mapping, default and Jackson 3 adapters, and reusable `AUTO`/`INDEX`/`SCAN` Search projection assertions. Legacy A2A coverage verifies canonical Console/RAD projection, exact-Version child publishers, SERVICE compatibility without Beta historical Naming dual-write, pre-registration, duplicate protection, multi-Version aggregation, and later canonical publication. An opt-in directed IT stops and restarts a real standalone server while the same SDK process verifies connection failure, protocol-neutral and legacy gRPC publication redo, HTTP `50404` replay, gRPC Watch ability renegotiation and resubscription with a new connection-scoped key, HTTP polling recovery, Search convergence, and later Version/Endpoint callbacks. | HTTP Batch Long Poll Watch and HTTP/AUTO Wire routing, management-metadata subscription, public local-selection helpers, legacy Naming serviceName dual-write, and all-HTTP dual identity headers remain deferred. Ability changes after a successful connection, individual frame loss/ACK ambiguity, non-50404 heartbeat failures, rollback, and redo races remain deterministic unit-test scenarios rather than unstable shared-server fault injection. |
