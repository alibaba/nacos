...
| `AiService` / `A2aService` | `AiServiceJavaSdkITCase` | Partial | Verifies factory creation, MCP release/query/subscribe, MCP latest and duplicate-version controlled error behavior, MCP endpoint register/deregister with returned detail state, A2A agent card release/query/subscribe, A2A latest-version and service endpoint registration scenarios, current-value listener callbacks, missing-resource nullable MCP/A2A/Prompt subscribe shapes, gRPC Skill/AgentSpec unsupported error mapping, missing skill download controlled exception, SDK validation for MCP/A2A/Prompt/Skill/AgentSpec required parameters, endpoint validation, batch endpoint version mismatch, and shutdown cleanup. | Direct MCP endpoint-spec release, all-version MCP endpoint behavior, A2A duplicate-version idempotency, A2A batch endpoint overwrite, functional Prompt/Skill/AgentSpec resource scenarios, and unsubscribe-stop callback behavior remain. |
