...
| Five-resource transport compatibility | Exercise Agent, MCP, Prompt, Skill, and AgentSpec through explicit `grpc`, explicit `http`, and `auto`, including one-shot reads/writes, subscriptions, and current unsupported branches. | Covered | `AiTransportResourceMatrixJavaSdkITCase` uses a real standalone server and isolated Maintainer SDK fixtures to verify every mode. Agent and MCP protocol-neutral operations follow the selected transport; Prompt follows the legacy proxy selected at construction; Skill ZIP download remains HTTP; and gRPC/AUTO Skill subscription plus AgentSpec load/subscription return controlled `SERVER_NOT_IMPLEMENTED` errors. |
| MCP release/query | New MCP, new version, duplicate version controlled error, latest-published-version lookup, explicit-version lookup, direct-online compatibility, managed draft creation, tool/resource/endpoint variants, invalid specification, and missing MCP behavior. | Covered | `AiServiceJavaSdkITCase` and `McpHttpClientJavaSdkITCase` verify gRPC and HTTP default/explicit `createDraft=false` direct-online behavior, managed `createDraft=true` draft-only behavior or its controlled pre-cutover gate, Tool/Resource/direct and auto-REF forms, duplicate errors, latest and exact query, invalid arguments, and absence before draft publication. Strict gRPC ability gating is also covered by focused client tests. |
