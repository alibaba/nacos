...
| `NamingMaintainerService` / `ServiceMaintainerService` / `InstanceMaintainerService` | `NamingMaintainerServiceMaintainerSdkITCase` | Partial | Verifies persistent service create/get/update/list/remove lifecycle, missing service controlled exception, invalid service parameter validation, persistent instance register/list/detail/update/partial-update/batch-metadata-update/batch-metadata-delete/deregister lifecycle, invalid instance parameter validation, default host/port wiring, and cleanup. | Cluster health, health-checker queries, subscribers, selector types, naming metrics/log operations, naming client diagnostics, and auth-enabled behavior remain pending for later batches. |
...
- `NamingMaintainerService` cluster, health, ops, subscriber, selector, and
  client diagnostics operations
