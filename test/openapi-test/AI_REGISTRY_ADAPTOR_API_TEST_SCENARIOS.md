...
| `ArdExceptionHandlerTest`, `AuthFilterTest`, `ArdOpenApiContractTest` | Errors from ARD controller and authentication operations | Covered | Verifies HTTP 401 for missing/rejected credentials and the exact `{errorCode, message}` body without the Nacos `Result<T>` envelope. |
