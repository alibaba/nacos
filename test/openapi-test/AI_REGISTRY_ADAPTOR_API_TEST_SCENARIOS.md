...
| --- | --- | --- | --- |
| `ArdSearchServiceImplTest`, `ArdSearchControllerTest`, `ArdOpenApiContractTest` | `GET /v3/ai/ard`, `GET /v3/ai/ard/agents`, `POST /v3/ai/ard/search`, `POST /v3/ai/ard/explore` | Partial | Verifies `items`, integral relevance scores, URI-valued `source`, optional trust identity, pagination, ISO date/instant list filters, standard nested and extension field paths, and the Search/List/Explore shapes pinned by the vendored OpenAPI. Agent scenarios verify one logical identifier, deterministic pure-A2A versus multi-protocol default representations, A2A/Nacos media-type filtering through `artifactKinds`, representation-aware type facets, and exact Version/digest artifact URLs without duplicating one Agent before totals or pagination. A live standalone adaptor conformance suite remains to be added. |
...
| `ArdArtifactServiceTest`, `ArdSearchControllerTest`, `ArdWebContextIsolationTest` | `GET /v3/ai/ard/artifacts` | Covered | Verifies complete Skill ZIP generation and exact Agent artifact resolution. Agent cases cover native A2A Agent Card and schema-limited Nacos Agent representations, required digest/representation parameters, disabled or missing Agent, offline or missing Version, digest mismatch, unavailable representation, and propagated unexpected persistence failures. Also proves that the adaptor root context owns the artifact route while the main server at `/nacos` alone owns `/v3/client/ai/skills`. |
