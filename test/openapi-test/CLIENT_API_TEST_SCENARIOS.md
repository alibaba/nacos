...
| `SkillClientOpenApiITCase` | `GET /v3/client/ai/skills`<br>`GET /v3/client/ai/skills/subscriptions` | Covered | Downloads online skills as ZIP by latest, version, and label with resource entries; covers namespace defaulting, version-over-label priority, missing skillName, absent skill, unknown version/label, controlled not-found JSON for download failures, and client subscription reads from runtime config dump/cache for the current request user. |
