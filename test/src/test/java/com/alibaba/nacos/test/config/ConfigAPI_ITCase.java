...
public class ConfigAPI_ITCase {
	...
	public void cleanup() throws Exception {
		...
		try {
			List<String> params = Arrays
					.asList("dataId", dataId, "group", group, "beta", "true");
			result = agent.httpDelete(CONFIG_CONTROLLER_PATH + "/", null, params,
					agent.getEncode(), TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
			Assert.assertEquals(true,
					JSON.parseObject(result.content).getBoolean("data"));
		}
		catch (Exception e) {
			e.printStackTrace();
			Assert.fail();
	...
	public void nacos_getconfig_1() throws Exception {
		final String dataId = "nacos_getconfig_1";
		final String group = "nacos_getconfig_1";
	...
	public void nacos_getconfig_2() throws Exception {
		final String dataId = "nacos_getconfig_2";
		final String group = "nacos_getconfig_2";
		String content = iconfig.getConfig(dataId, group, TIME_OUT);
	...
	public void nacos_openAPI_detailConfig_1() {
		...
		try {
			...
			List<String> params = Arrays
					.asList("dataId", dataId, "group", group, "show", "all");
			result = agent
					.httpGet(CONFIG_CONTROLLER_PATH, null, params, agent.getEncode(),
							TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);

			Assert.assertEquals(content,
					JSON.parseObject(result.content).getString("content"));
		}
		catch (Exception e) {
			Assert.fail();
		}
	...
	public void nacos_openAPI_catalog() {
		...
		try {
			...
			result = agent.httpGet(CONFIG_CONTROLLER_PATH + "/catalog", null, params,
					agent.getEncode(), TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
	...
	public void nacos_openAPI_queryBeta_1() {
		...
		try {
			...
			List<String> params1 = Arrays
					.asList("dataId", dataId, "group", group, "content", content);
			result = agent.httpPost(CONFIG_CONTROLLER_PATH + "/", headers, params1,
					agent.getEncode(), TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
			...
			List<String> params = Arrays
					.asList("dataId", dataId, "group", group, "beta", "true");
			result = agent.httpGet(CONFIG_CONTROLLER_PATH + "/", null, params,
					agent.getEncode(), TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
			Assert.assertEquals(content,
					JSON.parseObject(result.content).getJSONObject("data")
							.getString("content"));
			// delete data
			result = agent.httpDelete(CONFIG_CONTROLLER_PATH + "/", null, params,
					agent.getEncode(), TIME_OUT);
	...
	public void nacos_openAPI_fuzzySearchConfig() {
		...
		try {
			...
			List<String> params = Arrays
					.asList("dataId", dataId, "group", group, "pageNo", "1", "pageSize",
							"10", "search", "blur");
			result = agent.httpGet(CONFIG_CONTROLLER_PATH + "/", null, params,
					agent.getEncode(), TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
			...
			Assert.assertTrue(
					JSON.parseObject(result.content).getIntValue("totalCount") >= 1);
			Assert.assertTrue(JSON.parseObject(result.content).getJSONArray("pageItems")
					.getJSONObject(0).getString("content").startsWith(content));
		}
		catch (Exception e) {
			Assert.fail();
		}
	...
	public void nacos_openAPI_fuzzySearchConfig_1() {
		...
		try {
			...
			List<String> params = Arrays
					.asList("dataId", dataId + "*", "group", group + "*", "pageNo", "1",
							"pageSize", "10", "search", "blur");
			result = agent.httpGet(CONFIG_CONTROLLER_PATH + "/", null, params,
					agent.getEncode(), TIME_OUT);
			...
			Assert.assertTrue(
					JSON.parseObject(result.content).getIntValue("totalCount") >= 1);
			Assert.assertEquals(content,
					JSON.parseObject(result.content).getJSONArray("pageItems")
	...
	public void nacos_openAPI_searchConfig() {
		...
		try {
			...
			Assert.assertEquals(1,
					JSON.parseObject(result.content).getIntValue("totalCount"));
			Assert.assertEquals(content,
					JSON.parseObject(result.content).getJSONArray("pageItems")
							.getJSONObject(0).getString("content"));
	...
	public void nacos_openAPI_searchConfig_2() {
		...
		try {
			...
			List<String> params = Arrays
					.asList("dataId", dataId, "group", group, "pageNo", "1", "pageSize",
							"10", "search", "accurate");
			result = agent.httpGet(CONFIG_CONTROLLER_PATH + "/", null, params, "utf-8",
					TIME_OUT);
			Assert.assertEquals(HttpURLConnection.HTTP_OK, result.code);
			Assert.assertEquals(1,
					JSON.parseObject(result.content).getIntValue("totalCount"));
