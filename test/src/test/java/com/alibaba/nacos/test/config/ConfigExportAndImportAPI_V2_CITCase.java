...
@SpringBootTest(classes = Nacos.class, properties = {"server.servlet.context-path=/nacos",
        "server.port=7003"}, webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT)
public class ConfigExportAndImportAPI_V2_CITCase {
    ...
    public static void cleanClientCache() throws Exception {
        ...
        ConfigCleanUtils.changeToNewTestNacosHome(ConfigExportAndImportAPI_V2_CITCase.class.getSimpleName());
    }
    ...
    public void testExportV2ByIds() {
        ...
        String exportByIdsUrl = "?export_v2=true&tenant=&group=&appName=&ids=" + id1 + "," + id2;
        ...
        String config1Name = "group-" + config1.get("group").textValue() + "/" + config1.get("dataId").textValue();
        String config2Name = "group-" + config2.get("group").textValue() + "/" + config2.get("dataId").textValue();
    ...
    public void testExportByGroup() {
        ...
        JsonNode config1 = resultConfigs.get(0);
        JsonNode config2 = resultConfigs.get(1);
        String exportByIdsUrl = "?export_v2=true&tenant=&group=EXPORT_IMPORT_TEST_GROUP&appName=&ids=";
        byte[] zipData = httpClient.download(SERVER_ADDR + CONFIG_CONTROLLER_PATH + exportByIdsUrl, null);
        ZipUtils.UnZipResult unZiped = ZipUtils.unzip(zipData);
        List<ZipUtils.ZipItem> zipItemList = unZiped.getZipItemList();
        Assert.assertEquals(2, zipItemList.size());
        ...
        String metaDataName = "EXPORT_IMPORT_TEST_GROUP|testHasAppname1.properties";
        String appName = metaData.get(metaDataName)[2];
    ...
    public void testExportByGroupAndApp() {
        ...
        JsonNode config1 = resultConfigs.get(0);
        String exportByIdsUrl = "?export_v2=true&tenant=&group=EXPORT_IMPORT_TEST_GROUP&appName=testApp1&ids=";
        byte[] zipData = httpClient.download(SERVER_ADDR + CONFIG_CONTROLLER_PATH + exportByIdsUrl, null);
        ZipUtils.UnZipResult unZiped = ZipUtils.unzip(zipData);
        List<ZipUtils.ZipItem> zipItemList = unZiped.getZipItemList();
    ...
    public void testExportAll() {
        String exportByIdsUrl = "?export_v2=true&tenant=&group=&appName=&ids=";
        byte[] zipData = httpClient.download(SERVER_ADDR + CONFIG_CONTROLLER_PATH + exportByIdsUrl, null);
        ZipUtils.UnZipResult unZiped = ZipUtils.unzip(zipData);
        List<ZipUtils.ZipItem> zipItemList = unZiped.getZipItemList();
        String config1Name = "group-" + "EXPORT_IMPORT_TEST_GROUP/testNoAppname1.yml";
        String config2Name = "group-" + "TEST1_GROUP/testNoAppname2.txt";
        String config3Name = "group-" + "EXPORT_IMPORT_TEST_GROUP/testHasAppname1.properties";
        int successCount = 0;
        for (ZipUtils.ZipItem zipItem : zipItemList) {
    ...
    public void testImport() {
        ...
        zipItemList.add(new ZipUtils.ZipItem("group-" + "TEST_IMPORT/test1.yml", "test: test1"));
        zipItemList.add(new ZipUtils.ZipItem("group-" + "TEST_IMPORT/test2.txt", "test: test1"));
        zipItemList.add(new ZipUtils.ZipItem("group-" + "TEST_IMPORT/test3.properties", "test.test1.value=test"));
        zipItemList.add(new ZipUtils.ZipItem("group-" + "TEST_IMPORT/test4.cfg", "test.test4.value=test"));
        
        String metaDataStr = "# TEST_IMPORT|test1.yml|testApp1|yml\r\n\r\n" // comment skip
                + "TEST_IMPORT|test1.yml|testApp1|yml\r\n" // test1
                + "TEST_IMPORT|test2.txt|testApp2|properties\r\n" // test 2
                + "TEST_IMPORT|test4.cfg||cfg"; // test4
        ...
        JsonNode resultObj = JacksonUtils.toObj(queryResult);
        JsonNode resultConfigs = resultObj.get("pageItems");
        Assert.assertEquals(4, resultConfigs.size());
    ...
    // group|dataId -> [group|dataId|app_name|type]
    private Map<String, String[]> processMetaData(ZipUtils.ZipItem metaItem) {
